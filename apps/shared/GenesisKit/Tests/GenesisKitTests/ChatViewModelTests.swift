...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> GenesisChatHistoryPayload
{
    GenesisChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> GenesisChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> GenesisChatSessionEntry
{
    GenesisChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> GenesisChatModelChoice {
    GenesisChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}
...
private func makeViewModel(
    ...
    modelResponses: [[GenesisChatModelChoice]] = [],
    ...
    -> (TestChatTransport, GenesisChatViewModel)
{
...
private func sendUserMessage(_ vm: GenesisChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, GenesisChatTransport {
    ...
    private let modelResponses: [[GenesisChatModelChoice]]
    ...
    func listModels() async throws -> [GenesisChatModelChoice] {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = GenesisChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: GenesisChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = GenesisChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: GenesisChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(GenesisChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == GenesisChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = GenesisChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: GenesisChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = GenesisChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = GenesisChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
