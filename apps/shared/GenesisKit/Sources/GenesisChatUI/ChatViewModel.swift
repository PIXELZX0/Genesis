...
public final class GenesisChatViewModel {
    ...
    public private(set) var modelChoices: [GenesisChatModelChoice] = []
    ...
    private var sessionDefaults: GenesisChatSessionsDefaults?
    ...
    public init(
        sessionKey: String,
        transport: any GenesisChatTransport,
    ...
    private static func messageContentFingerprint(for message: GenesisChatMessage) -> String {
    ...
    private static func userRefreshIdentityKey(for message: GenesisChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = GenesisChatSessionEntry(
                key: current.key,
                kind: current.kind,
