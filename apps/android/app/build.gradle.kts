...
val androidStoreFile = providers.gradleProperty("GENESIS_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("GENESIS_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("GENESIS_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("GENESIS_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set GENESIS_ANDROID_STORE_FILE, " +
            "GENESIS_ANDROID_STORE_PASSWORD, GENESIS_ANDROID_KEY_ALIAS, and " +
            "GENESIS_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/GenesisKit/Sources/GenesisKit/Resources")
        }
    }
    ...
    defaultConfig {
        ...
        minSdk = 31
        targetSdk = 36
        versionCode = 2026051002
        versionName = "2026.5.10-2"
        ndk {
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "GENESIS_ENABLE_SMS", "false")
            buildConfigField("boolean", "GENESIS_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "GENESIS_ENABLE_SMS", "true")
            buildConfigField("boolean", "GENESIS_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "genesis-$versionName-$buildType.apk"
                    } else {
                        "genesis-$versionName-$flavorName-$buildType.apk"
                    }
