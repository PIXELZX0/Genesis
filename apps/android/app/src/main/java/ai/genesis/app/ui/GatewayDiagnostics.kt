...
internal fun genesisAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Genesis Android gateway connection failure.
    ...
    - tell me whether `genesis devices list` should show a pending pairing request
    - if more signal is needed, ask for `genesis qr --json`, `genesis devices list`, and `genesis nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${genesisAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Genesis gateway diagnostics", report))
