...
private const val NOTIFICATION_CHANNEL_BASE_ID = "genesis.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Genesis Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Genesis Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Genesis Active")
      }
