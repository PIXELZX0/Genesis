...
import ai.genesis.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      // Camera commands
      GenesisCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      ...
      // Device commands
      GenesisDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      GenesisDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      GenesisDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      GenesisDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      GenesisNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      GenesisNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      GenesisSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.genesis.app.protocol.GenesisPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      GenesisContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      GenesisContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      GenesisCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      GenesisCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      GenesisMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      GenesisMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      GenesisSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      GenesisSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      GenesisCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
