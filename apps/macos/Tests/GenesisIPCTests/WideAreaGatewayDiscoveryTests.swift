...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("GENESIS_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("GENESIS_WIDE_AREA_DOMAIN", "genesis.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("GENESIS_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("GENESIS_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._genesis-gw._tcp.genesis.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
