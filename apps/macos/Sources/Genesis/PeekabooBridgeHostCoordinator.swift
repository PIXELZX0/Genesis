...
final class PeekabooBridgeHostCoordinator {
    ...
    private static var genesisSocketPath: String {
        ...
        return Self.makeSocketPath(for: "Genesis", in: base)
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.genesisSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.genesisSocketPath)
        } catch {
