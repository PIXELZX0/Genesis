...
private let webChatThinkingLevelDefaultsKey = "genesis.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: GenesisChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> GenesisChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(GenesisChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            GenesisChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return GenesisChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    private static func mapModelChoice(_ model: GenesisProtocol.ModelChoice) -> GenesisChatModelChoice {
        GenesisChatModelChoice(
            modelID: model.id,
            name: model.name,
