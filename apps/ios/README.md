...
- Beta release uses canonical `ai.genesis.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `GenesisPushTransport=relay`, `GenesisPushDistribution=official`, and `GenesisPushAPNsEnvironment=production`.
...
- `GENESIS_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export GENESIS_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Genesis-<version>.ipa`
   - `apps/ios/build/beta/Genesis-<version>.app.dSYM.zip`
...
- Local/manual builds default to `GenesisPushTransport=direct` and `GenesisPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `GENESIS_APNS_TEAM_ID`, `GENESIS_APNS_KEY_ID`, and either `GENESIS_APNS_PRIVATE_KEY_P8` or `GENESIS_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.genesis/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `GENESIS_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `GenesisPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `genesis.json`. `GENESIS_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Genesis official builds and to ensure a
