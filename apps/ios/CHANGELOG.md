# Genesis iOS Changelog

## 2026.4.24 - 2026-04-24

Maintenance update for the current Genesis development release.

## 2026.4.23 - 2026-04-23

Maintenance update for the current Genesis development release.

## 2026.4.22 - 2026-04-22

Maintenance update for the current Genesis development release.

## 2026.4.21 - 2026-04-21

Maintenance update for the current Genesis development release.

## 2026.4.20 - 2026-04-20

Maintenance update for the current Genesis release.

## 2026.4.19 - 2026-04-19

Maintenance update for the current Genesis beta release.

## 2026.4.18 - 2026-04-18

Maintenance update for the current Genesis release.

## 2026.4.15 - 2026-04-15

Maintenance update for the current Genesis beta release.

## 2026.4.14 - 2026-04-14

Maintenance update for the current Genesis beta release.

## 2026.4.12 - 2026-04-12

Maintenance update for the current Genesis release.

## 2026.4.10 - 2026-04-10

Maintenance update for the current Genesis release.

## 2026.4.6 - 2026-04-06

First App Store release of Genesis for iPhone. Pair with your Genesis Gateway to use chat, voice, sharing, and device actions from iOS.
