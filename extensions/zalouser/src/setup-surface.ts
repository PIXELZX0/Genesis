...
function ensureZalouserPluginEnabled(cfg: GenesisConfig): GenesisConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("genesis directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
      ...
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("genesis directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
