...
import { loadConfig, type GenesisConfig } from "genesis/plugin-sdk/config-runtime";
import { encodePngRgba, fillPixel } from "genesis/plugin-sdk/media-runtime";
...
const LIVE_IMAGE_MODEL = process.env.GENESIS_LIVE_XAI_IMAGE_MODEL?.trim() || "grok-imagine-image";
const liveEnabled = XAI_API_KEY.trim().length > 0 && process.env.GENESIS_LIVE_TEST === "1";
const describeLive = liveEnabled ? describe : describe.skip;
const EMPTY_AUTH_STORE = { version: 1, profiles: {} } as const;

function createLiveConfig(): GenesisConfig {
...
describeLive("xai plugin live", () => {
  it("synthesizes TTS through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "Genesis xAI text to speech integration test OK.",
      cfg,
      providerConfig: {
        apiKey: XAI_API_KEY,
        baseUrl: "https://api.x.ai/v1",
    ...
    const telephony = await speechProvider.synthesizeTelephony?.({
      text: "Genesis xAI telephony check OK.",
      cfg,
      providerConfig: {
        apiKey: XAI_API_KEY,
        baseUrl: "https://api.x.ai/v1",
  ...
  it("transcribes audio through the registered media provider", async () => {
    ...
    const speechProvider = requireRegisteredProvider(speechProviders, "xai");
    const cfg = createLiveConfig();
    const phrase = "Genesis xAI speech to text integration test OK.";
    ...
    expect(compact).toContain("genesis");
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const speechProvider = requireRegisteredProvider(speechProviders, "xai");
    const cfg = createLiveConfig();
    const phrase = "Genesis xAI realtime transcription integration test OK.";
