...
vi.mock("genesis/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("genesis/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
vi.mock("genesis/plugin-sdk/approval-gateway-runtime", () => ({
  resolveApprovalOverGateway: (arg: unknown) => resolveApprovalOverGatewayMock(arg),
}));

vi.mock("genesis/plugin-sdk/security-runtime", () => ({
...
describe("registerSlackInteractionEvents", () => {
  ...
  it("registers a matcher that accepts plugin action ids beyond the Genesis prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("genesis:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
    ...
    await handler!({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "genesis:reply_button" }],
            },
          ],
        },
      },
      action: {
        type: "button",
        action_id: "genesis:reply_button",
        block_id: "reply_actions",
        value: "codex",
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"genesis:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
