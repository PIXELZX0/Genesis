...
describe("Slack native command argument menus", () => {
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^genesis_cmdarg/),
      ),
    ).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("genesis_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("genesis_cmdarg_0_1");
