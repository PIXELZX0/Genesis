import { hasConfiguredSecretInput } from "genesis/plugin-sdk/secret-input";
import {
...
import { formatDocsLink } from "genesis/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: GenesisConfig, accountId: string): GenesisConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: GenesisConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: GenesisConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: GenesisConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: GenesisConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: GenesisConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: GenesisConfig; accountId: string }) =>
    ...
    disable: (cfg: GenesisConfig) => setSetupChannelEnabled(cfg, channel, false),
