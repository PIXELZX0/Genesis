...
const GENESIS_OPENSHELL_E2E = process.env.GENESIS_E2E_OPENSHELL === "1";
const GENESIS_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const GENESIS_OPENSHELL_COMMAND = process.env.GENESIS_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "genesis-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && GENESIS_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: GENESIS_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(GENESIS_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(GENESIS_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-openshell-e2e-"));
      ...
      const gatewayName = `genesis-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `genesis-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/genesis-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: GENESIS_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("genesis-openshell-e2e");
