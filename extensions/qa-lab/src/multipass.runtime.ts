...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/genesis-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/genesis";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/genesis-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `genesis-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "GENESIS_CONFIG_PATH" &&
          key !== "GENESIS_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `GENESIS_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `GENESIS_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm genesis qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredGenesisTmpDir(), `${plan.vmName}-qa-suite-`),
  );
