...
const QA_LAB_UI_OVERLAY_DIR = "/opt/genesis-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        GENESIS_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      GENESIS_CONFIG_PATH: /tmp/genesis/genesis.json
      ...
      GENESIS_PROFILE: ""
    volumes:
      - ./state:/opt/genesis-scaffold:ro
      - ${repoMount}:/opt/genesis-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/genesis/workspace /tmp/genesis/state && cp /opt/genesis-scaffold/genesis.json /tmp/genesis/genesis.json && cp -R /opt/genesis-scaffold/seed-workspace/. /tmp/genesis/workspace/ && ln -snf /opt/genesis-repo /tmp/genesis/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t genesis:qa-local-prebaked --build-arg GENESIS_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "genesis:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "genesis.json"),
  ];
