...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "genesis.json"),
    ...
    expect(compose).toContain('      - "http://genesis-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/genesis-qa-lab-ui"');
    expect(compose).toContain(":/opt/genesis-repo:ro");
    expect(compose).toContain("./state:/opt/genesis-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/genesis-scaffold/seed-workspace/. /tmp/genesis/workspace/",
    );
    expect(compose).toContain("GENESIS_CONFIG_PATH: /tmp/genesis/genesis.json");
    expect(compose).toContain("GENESIS_STATE_DIR: /tmp/genesis/state");
    expect(compose).toContain('GENESIS_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("GENESIS_GATEWAY_TOKEN=qa-token");
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/genesis",
        imageName: "genesis:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("genesis:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t genesis:qa-local-prebaked --build-arg GENESIS_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/genesis",
      ),
    ]);
  });
});
