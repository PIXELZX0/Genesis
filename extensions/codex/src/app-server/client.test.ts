...
describe("CodexAppServerClient", () => {
  ...
  it("blocks unsupported app-server versions during initialize", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "genesis/0.117.9 (macOS; test)" },
    });
  ...
  it("reads the Codex version from the app-server user agent", () => {
    ...
    expect(readCodexVersionFromUserAgent("genesis/0.118.0 (macOS; test)")).toBe("0.118.0");
    ...
    expect(readCodexVersionFromUserAgent("genesis/0.118.0abc")).toBeUndefined();
