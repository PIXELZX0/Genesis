...
import { resolveUserPath } from "genesis/plugin-sdk/agent-harness";
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.GENESIS_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.GENESIS_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
