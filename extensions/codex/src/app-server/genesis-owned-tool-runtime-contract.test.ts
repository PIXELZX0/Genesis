...
import {
  ...
  mediaToolResult,
  resetGenesisOwnedToolHooks,
  textToolResult,
} from "../../../../test/helpers/agents/genesis-owned-tool-runtime-contract.js";
...
describe("Genesis-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetGenesisOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installGenesisOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installGenesisOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installGenesisOwnedToolHooks();
