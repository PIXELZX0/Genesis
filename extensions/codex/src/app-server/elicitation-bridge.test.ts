...
vi.mock("genesis/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("genesis/plugin-sdk/agent-harness-runtime")>()),
...
function buildCurrentCodexApprovalElicitation() {
  return {
    ...
    _meta: {
      ...
      tool_params_display: [
        { name: "repo", display_name: "Repository", value: "PIXELZX0/Genesis" },
      ],
    },
    requestedSchema: {
      type: "object",
...
describe("Codex app-server elicitation bridge", () => {
  ...
  it("accepts current Codex MCP approval elicitations with an empty form schema", async () => {
    ...
    expect(approvalRequest.description).toContain("Repository: PIXELZX0/Genesis");
  });
          ...
          tool_params_display: [
            {
              ...
              value: "\u001b]8;;https://evil.example\u001b\\PIXELZX0/Genesis\u001b]8;;\u001b\\",
            },
          ],
        },
        requestedSchema: {
  ...
  it("falls back to stable names when display labels sanitize to empty", async () => {
    ...
    expect(approvalRequest.description).toContain("- repo: PIXELZX0/Genesis");
