...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("genesis.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this Genesis session yet.");
  }
  return binding;
}
