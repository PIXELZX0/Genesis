...
import { sleep } from "genesis/plugin-sdk/text-runtime";
...
vi.mock("genesis/plugin-sdk/text-runtime", async () => {
  const actual = await vi.importActual<typeof import("genesis/plugin-sdk/text-runtime")>(
    "genesis/plugin-sdk/text-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
