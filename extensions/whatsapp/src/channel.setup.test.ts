  ...
  return {
    ...
    splitSetupEntries: (raw: string) =>
      raw
        ...
        .filter(Boolean),
    setSetupChannelEnabled: (cfg: GenesisConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<GenesisConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
...
async function runConfigureWithHarness(params: {
  ...
  const result = await finalizeWhatsAppSetup({
    cfg: params.cfg ?? ({} as GenesisConfig),
