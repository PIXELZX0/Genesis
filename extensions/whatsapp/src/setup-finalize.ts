...
import type { ChannelSetupWizard } from "genesis/plugin-sdk/setup";
...
type WhatsAppConfig = NonNullable<NonNullable<GenesisConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: GenesisConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: GenesisConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: GenesisConfig, accountId: string): string {
  if (
