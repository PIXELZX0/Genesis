...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/genesis-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("genesis/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("genesis/plugin-sdk/conversation-runtime")>(
    "genesis/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("genesis/plugin-sdk/reply-runtime"),
      import("genesis/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
