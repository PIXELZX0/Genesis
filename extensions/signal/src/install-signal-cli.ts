...
import { runPluginCommandWithTimeout } from "genesis/plugin-sdk/run-command";
import type { RuntimeEnv } from "genesis/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "genesis/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredGenesisTmpDir(), "genesis-signal-"));
