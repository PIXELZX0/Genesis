...
The bundled ACPX extension is a thin Genesis wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `genesis/acpx`, not in this extension.
...
Use this flow when Genesis needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `genesis/acpx` repo first.
2. In Genesis, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Genesis and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Genesis ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Genesis-specific glue, move it to `genesis/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
