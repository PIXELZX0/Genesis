...
import { formatPluginConfigIssue } from "genesis/plugin-sdk/extension-shared";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "genesis-plugin-tools";
export const ACPX_GENESIS_TOOLS_MCP_SERVER_NAME = "genesis-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "genesis.plugin.json")) &&
...
function resolveAcpxPluginRootFromGenesisLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Genesis layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromGenesisLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveGenesisRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const genesisRoot = resolveGenesisRoot(pluginRoot);
  const distEntry = path.join(genesisRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(genesisRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
export function resolveGenesisToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const genesisRoot = resolveGenesisRoot(pluginRoot);
  const distEntry = path.join(genesisRoot, "dist", "mcp", "genesis-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(genesisRoot, "src", "mcp", "genesis-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.genesisToolsMcpBridge && resolved[ACPX_GENESIS_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_GENESIS_TOOLS_MCP_SERVER_NAME} is reserved when genesisToolsMcpBridge=true`,
    );
  }
  ...
  if (params.genesisToolsMcpBridge) {
    resolved[ACPX_GENESIS_TOOLS_MCP_SERVER_NAME] = resolveGenesisToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
export function resolveAcpxPluginConfig(params: {
  ...
  const genesisToolsMcpBridge = normalized.genesisToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    genesisToolsMcpBridge,
