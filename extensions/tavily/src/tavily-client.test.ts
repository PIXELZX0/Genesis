...
vi.mock("genesis/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: genesis", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "genesis" });
  });

  it("runTavilyExtract sends X-Client-Source: genesis", async () => {
