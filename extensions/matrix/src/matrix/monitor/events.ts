...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'genesis matrix verify status --verbose --account ${accountId}' and 'genesis matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Genesis runtime is using the same account. " +
    `Check 'genesis matrix verify status --verbose --account ${accountId}' and 'genesis matrix devices list --account ${accountId}'.`
  );
}
