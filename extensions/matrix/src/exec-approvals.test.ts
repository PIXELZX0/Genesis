...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<GenesisConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<GenesisConfig["channels"]>["matrix"]>>,
): GenesisConfig {
  return {
    channels: {
      matrix: {
