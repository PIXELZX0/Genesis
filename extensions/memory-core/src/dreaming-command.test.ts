...
import type { GenesisConfig, GenesisPluginApi } from "genesis/plugin-sdk/memory-core";
...
function resolveStoredDreaming(config: GenesisConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: GenesisConfig = {}) {
  const registered: { command?: GenesisPluginCommandDefinition } = {};
  let runtimeConfig: GenesisConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: GenesisConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: GenesisPluginCommandDefinition) => {
