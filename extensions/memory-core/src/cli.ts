...
import {
  ...
  formatHelpExamples,
  theme,
} from "genesis/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          ["genesis memory status --deep", "Probe embedding provider readiness."],
          ["genesis memory index --force", "Force a full reindex."],
          ['genesis memory search "meeting notes"', "Quick search using positional query."],
          [
            'genesis memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `genesis memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "genesis memory rem-backfill --path ./memory",
