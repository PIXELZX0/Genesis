...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("genesis.memoryEmbeddingProviders");
...
vi.mock("genesis/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("genesis/plugin-sdk/memory-core-host-engine-foundation")
  >("genesis/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("genesis/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("genesis/plugin-sdk/file-lock")>(
    "genesis/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { GenesisConfig } from "genesis/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
