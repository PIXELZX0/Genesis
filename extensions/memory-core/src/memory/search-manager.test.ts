...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "genesis/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("genesis/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): GenesisConfig {
  return {
    agents: {
      defaults: {
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("genesis.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-qmd-workspace-"));
