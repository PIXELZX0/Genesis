...
import { resolveMemoryRemDreamingConfig } from "genesis/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: GenesisConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredGenesisTmpDir(), "genesis-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: GenesisConfig): string {
...
function resolveAgent(cfg: GenesisConfig, agent?: string) {
...
function resolveAgentIds(cfg: GenesisConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: genesis memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredGenesisTmpDir(), "genesis-rem-backfill-"),
      );
      try {
