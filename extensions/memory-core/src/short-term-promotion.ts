...
import { formatMemoryDreamingDay } from "genesis/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "genesis/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "genesis-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*genesis-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*genesis-memory-promotion:([^\n]+?)\s*-->/gi);
