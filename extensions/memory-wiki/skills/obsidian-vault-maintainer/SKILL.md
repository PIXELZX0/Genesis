...
- Start from `genesis wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `genesis wiki obsidian status` before shelling out, then prefer the dedicated helpers like `genesis wiki obsidian search`, `genesis wiki obsidian open`, `genesis wiki obsidian command`, and `genesis wiki obsidian daily`.
