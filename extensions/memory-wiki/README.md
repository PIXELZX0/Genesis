...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Genesis**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.genesis-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
genesis wiki get entity.alpha --from 1 --lines 80

genesis wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

genesis wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

genesis wiki bridge import
genesis wiki unsafe-local import

genesis wiki obsidian status
genesis wiki obsidian search "alpha"
genesis wiki obsidian open syntheses/alpha-summary.md
genesis wiki obsidian command workspace:quick-switcher
genesis wiki obsidian daily
```
...
- The compiled agent digests in `.genesis-wiki/cache/agent-digest.json` and `.genesis-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
