...
vi.mock("genesis/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("genesis/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<GenesisConfig["channels"]>["discord"]>["execApprovals"],
): GenesisConfig {
  return {
    channels: {
      discord: {
