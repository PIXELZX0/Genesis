...
vi.mock("genesis/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("genesis/plugin-sdk/reply-dispatch-runtime")>(
...
vi.mock("genesis/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): GenesisConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("genesis/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
