import { DEFAULT_EMOJIS } from "genesis/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/genesis-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("genesis/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("genesis/plugin-sdk/conversation-runtime");
...
const sessionStoreRuntimeModule = await import("genesis/plugin-sdk/session-store-runtime");
...
describe("processDiscordMessage session routing", () => {
  ...
  it("prefers bound session keys and sets MessageThreadId for bound thread messages", async () => {
    const threadBindings = createThreadBindingManager({
      cfg: {} as import("genesis/plugin-sdk/config-runtime").GenesisConfig,
      accountId: "default",
      persist: false,
      enableSweeper: false,
    });
