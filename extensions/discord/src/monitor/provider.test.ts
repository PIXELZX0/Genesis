...
let runtimeEnvModule: typeof import("genesis/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): GenesisConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("genesis/plugin-sdk/plugin-runtime", async () => {
      const actual = await vi.importActual<typeof import("genesis/plugin-sdk/plugin-runtime")>(
        "genesis/plugin-sdk/plugin-runtime",
      );
      return {
        ...actual,
    ...
    runtimeEnvModule = await import("genesis/plugin-sdk/runtime-env");
  ...
  it("passes Genesis EventQueue defaults to Carbon Client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
