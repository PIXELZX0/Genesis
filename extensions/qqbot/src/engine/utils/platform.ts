 ...
 * `genesis/plugin-sdk`. The temp-directory fallback is delegated
...
/** Return a path under `~/.genesis/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".genesis", "qqbot", ...subPaths);
}

/** Return a path under `~/.genesis/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.genesis/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Genesis's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".genesis", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.genesis/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.genesis/media`, Genesis's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `genesis/src/media/local-roots.ts`). Using it as a
 ...
 */
export function getGenesisMediaDir(): string {
  return path.join(getHomeDir(), ".genesis", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".genesis", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Genesis's shared `~/.genesis/media`
  ...
  const allowedRoots = [getGenesisMediaDir(), getQQBotMediaPath()];
