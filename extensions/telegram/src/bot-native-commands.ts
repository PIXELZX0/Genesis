...
import type { ChannelGroupPolicy } from "genesis/plugin-sdk/config-runtime";
import type {
  ReplyToMode,
...
import { getRuntimeConfigSnapshot } from "genesis/plugin-sdk/runtime-config-snapshot";
...
type TelegramChunkMode = ReturnType<
  typeof import("genesis/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("genesis/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): GenesisConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: GenesisConfig): TelegramAccountConfig => {
    try {
