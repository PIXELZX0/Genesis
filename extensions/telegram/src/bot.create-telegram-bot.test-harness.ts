...
import type { GetReplyOptions, MsgContext } from "genesis/plugin-sdk/reply-runtime";
import type { MockFn } from "genesis/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("genesis/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("genesis/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("genesis/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("genesis/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/genesis-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("genesis/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: GenesisConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: GenesisConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: GenesisConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: GenesisConfig) => {
