import type { TelegramNetworkConfig } from "genesis/plugin-sdk/config-runtime";
...
let isWSL2Sync: typeof import("genesis/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  ({ isWSL2Sync } = await import("genesis/plugin-sdk/runtime-env"));
  ({
...
describe("resolveTelegramDnsResultOrderDecision", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: { GENESIS_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:GENESIS_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { GENESIS_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
