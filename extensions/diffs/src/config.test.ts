...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/genesis/",
      }),
    ).toBe("https://example.com/genesis");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/genesis?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/genesis#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/genesis/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/genesis",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/genesis/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("genesisDiffsReady");
