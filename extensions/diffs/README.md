...
Read-only diff viewer plugin for **Genesis** agents.
...
Set plugin-wide defaults in `~/.genesis/genesis.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/genesis",
        },
      },
    },
  },
...
Before:
Genesis supports plugins.

After:
Genesis supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/genesis-diffs`).
