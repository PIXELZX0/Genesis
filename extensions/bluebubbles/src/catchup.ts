...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit GENESIS_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.GENESIS_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.genesis.
  // Use resolvePreferredGenesisTmpDir + string concat (mirrors
  ...
  // Canonical Genesis state dir: honors `~` expansion + legacy/new
