...
function localGenesisProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "genesis",
...
function createLocalGenesisResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localGenesisProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveGenesisUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-reset-"));
    const profileDir = path.join(tempRoot, "genesis");
    ...
    const ops = createLocalGenesisResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveGenesisUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-reset-no-own-"));
    const profileDir = path.join(tempRoot, "genesis");
    fs.mkdirSync(profileDir, { recursive: true });
