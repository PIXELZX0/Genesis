...
let tmpDirModule: typeof import("../infra/tmp-genesis-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-genesis-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredGenesisTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredGenesisTmpDir,
    );
  ...
  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".genesis-output-");
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "genesis-preferred", "downloads")),
    );
