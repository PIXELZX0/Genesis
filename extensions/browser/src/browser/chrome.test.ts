...
type StopChromeTarget = Parameters<typeof stopGenesisChrome>[0];
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildGenesisChromeLaunchArgs({
      ...
      userDataDir: "/tmp/genesis-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/genesis-test-user-data");
  });
});
