import type { GenesisPluginSecurityAuditContext } from "genesis/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "genesis/plugin-sdk/secret-input";
import { formatCliCommand } from "genesis/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "genesis/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: GenesisPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("genesis security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.GENESIS_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.GENESIS_GATEWAY_PASSWORD) ||
