...
- Extension production code should import from `genesis/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `genesis.plugin.json` and the package
  `genesis` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `genesis/plugin-sdk/*`.
