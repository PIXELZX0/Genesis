...
import { canResolveEnvSecretRefInReadOnlyPath } from "genesis/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "genesis/plugin-sdk/secret-input";
...
export function resolveFirecrawlSearchConfig(cfg?: GenesisConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: GenesisConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: GenesisConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: GenesisConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: GenesisConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: GenesisConfig, override?: number): number {
