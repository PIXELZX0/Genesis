...
export function buildComfyConfig(config: Record<string, unknown>): GenesisConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): GenesisConfig {
  return {
    models: {
      providers: {
        comfy: config,
