import { formatCliCommand, parseDurationMs } from "genesis/plugin-sdk/cli-runtime";
import type {
  GenesisPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type GenesisConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "genesis/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  "Anthropic setup-token auth is supported in Genesis.",
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("genesis models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("genesis models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
export function registerAnthropicPlugin(api: GenesisPluginApi): void {
