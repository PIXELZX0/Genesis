...
function resolveKimiBaseUrl(kimi?: KimiConfig, genesisConfig?: GenesisConfig): string {
  ...
  const moonshotBaseUrl = genesisConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: GenesisConfig; searchConfig?: SearchConfigRecord },
