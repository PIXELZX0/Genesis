...
vi.mock("@earendil-works/pi-ai/providers/openai-codex", () => ({
  openaiCodexProvider: () => ({
    auth: { oauth: { refresh: runtimeMocks.refreshOpenAICodexToken } },
  }),
}));
...
describe("openai plugin", () => {
  ...
  it("bootstraps the env proxy dispatcher before refreshing codex oauth credentials", async () => {
    ...
    await expect(refreshOpenAICodexToken("refresh-token")).resolves.toStrictEqual(refreshed);
  ...
  it("includes the tagged GPT-5 behavior contract in the OpenAI prompt overlay", () => {
    ...
    expect(OPENAI_FRIENDLY_PROMPT_OVERLAY).not.toMatch(/heartbeat/i);
  });
