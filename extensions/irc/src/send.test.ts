...
vi.mock("genesis/plugin-sdk/config-runtime", async () => {
  const original = (await vi.importActual("genesis/plugin-sdk/config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("genesis/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("genesis/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
