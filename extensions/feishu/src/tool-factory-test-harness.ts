...
export function createToolFactoryHarness(cfg: GenesisPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<GenesisPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
