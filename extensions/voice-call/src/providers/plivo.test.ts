...
describe("PlivoProvider", () => {
  ...
  it("pins stored callback bases to publicUrl instead of request Host", () => {
    const provider = new PlivoProvider(
      ...
      {
        publicUrl: "https://voice.genesis.pixelzx.com/voice/webhook?provider=plivo",
      },
    );
    ...
    expect(callbackMap.get("call-uuid")).toBe("https://voice.genesis.pixelzx.com/voice/webhook");
  });
});
