...
- **Core CLI and gateway** — [PIXELZX0/Genesis](https://github.com/PIXELZX0/Genesis)
- **macOS desktop app** — [PIXELZX0/Genesis](https://github.com/PIXELZX0/Genesis) (apps/macos)
- **iOS app** — [PIXELZX0/Genesis](https://github.com/PIXELZX0/Genesis) (apps/ios)
- **Android app** — [PIXELZX0/Genesis](https://github.com/PIXELZX0/Genesis) (apps/android)
...
- Tested version details (Genesis version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Genesis. Showing that Genesis can reach a native parser is not enough by itself.
- Demonstrated impact tied to Genesis's documented trust boundaries.
- For exposed-secret reports: proof the credential is Genesis-owned (or grants access to Genesis-operated infrastructure/services).
...
Genesis does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-genesis-scopes` headers are ignored for that shared-secret path
...
- Genesis can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Genesis, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Genesis's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Genesis's trusted computing base for a gateway.
...
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Genesis production code or a published package artifact intended for users.
...
- Exposed secrets that are third-party/user-controlled credentials (not Genesis-owned and not granting access to Genesis-operated infrastructure/services) without demonstrated Genesis impact
...
Genesis's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Genesis separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Genesis can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
Genesis uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/genesis` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/genesis` (or `genesis-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Genesis-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Genesis temp helpers (`resolvePreferredGenesisTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-genesis-dir.ts`
