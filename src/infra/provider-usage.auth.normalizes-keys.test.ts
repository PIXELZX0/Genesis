...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "genesis-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".genesis");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".genesis", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      GENESIS_STATE_DIR: path.join(home, ".genesis"),
      ...env,
    };
