...
export function resolvePreferredGenesisTmpDir(
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[genesis] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Genesis temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Genesis temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_GENESIS_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_GENESIS_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_GENESIS_TMP_DIR)) {
      return POSIX_GENESIS_TMP_DIR;
    }
  ...
  try {
    ...
    chmodSync(POSIX_GENESIS_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_GENESIS_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_GENESIS_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_GENESIS_TMP_DIR;
