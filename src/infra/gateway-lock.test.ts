...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "genesis-gateway-lock-" });
let fixtureRoot = "";
...
describe("gateway lock", () => {
  ...
  it("blocks concurrent acquisition until release", async () => {
    ...
    const pending = acquireForTest(env, {
      timeoutMs: 15,
      readProcessCmdline: () => ["genesis", "gateway", "run"],
    });
  ...
  it("keeps lock when configured port is busy and owner pid is alive", async () => {
    ...
    try {
      const pending = acquireForTest(env, {
        ...
        platform: "darwin",
        port: 18789,
        readProcessCmdline: () => ["/usr/local/bin/genesis", "gateway", "run"],
      });
      await expect(pending).rejects.toBeInstanceOf(GatewayLockError);
    } finally {
      connectSpy.mockRestore();
  ...
  it("keeps lock on darwin when process cmdline is a gateway", async () => {
    ...
    const pending = acquireForTest(env, {
      ...
      platform: "darwin",
      port: 18789,
      readProcessCmdline: () => ["/usr/local/bin/genesis", "gateway", "run", "--port", "18789"],
    });
    await expect(pending).rejects.toBeInstanceOf(GatewayLockError);
