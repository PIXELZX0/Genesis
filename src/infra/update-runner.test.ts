...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "genesis-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:genesis-update-preflight-|ocu-pf-)/;
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "genesis.mjs")} doctor --non-interactive --fix`;
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g genesis@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g genesis@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "genesis.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "genesis.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("genesis-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "genesis.mjs")} doctor --non-interactive --fix`;
    ...
    expect(pnpmEnvPaths.some((value) => value.includes("genesis-update-pnpm-"))).toBe(true);
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    try {
      const runCommand = async (
        ...
        if (key === "pnpm install") {
          if (options?.cwd && /(?:genesis-update-preflight-|ocu-pf-)/.test(options.cwd)) {
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "genesis.mjs")} doctor --non-interactive --fix`;
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand:
        "npm i -g github:PIXELZX0/Genesis#main --no-fund --no-audit --loglevel=error",
      tag: "main",
    });
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g genesis@latest --no-fund --no-audit --loglevel=error",
      gitRootMode: "missing",
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g genesis@latest --no-fund --no-audit --loglevel=error");
  });
  ...
  it("fails global npm update when the installed version misses the requested correction", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: "npm i -g genesis@2026.3.23-2 --no-fund --no-audit --loglevel=error",
      tag: "2026.3.23-2",
    });
    ...
    expect(calls).toContain("npm i -g genesis@2026.3.23-2 --no-fund --no-audit --loglevel=error");
  });

  it("fails global npm update when bundled runtime sidecars are missing after install", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = "npm i -g genesis@latest --no-fund --no-audit --loglevel=error";
  ...
  it("prepends portable Git PATH for global Windows npm updates", async () => {
    ...
    const portableGitUsr = path.join(localAppData, "Genesis", "deps", "portable-git", "usr", "bin");
  ...
  it("uses GENESIS_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/genesis-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: expectedInstallCommand,
    ...
    await withEnvAsync(
      { GENESIS_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/genesis-next.tgz" },
      async () => {
        const result = await runWithCommand(runCommand, { cwd: pkgRoot });
        expect(result.status).toBe("ok");
      },
