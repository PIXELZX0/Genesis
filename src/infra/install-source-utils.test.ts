...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("genesis-install-source-utils-");
  const archivePath = path.join(cwd, "genesis-plugin-1.2.3.tgz");
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-genesis-archive.tgz",
...
describe("packNpmSpecToArchive", () => {
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "genesis-plugin-1.2.3.tgz");
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"genesis-plugin@1.2.3","name":"genesis-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "genesis-plugin",
        version: "1.2.3",
        resolvedSpec: "genesis-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@genesis/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@genesis/whatsapp", cwd);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "genesis-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
