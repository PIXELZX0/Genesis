...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/genesis-state", "devices")).toEqual({
      dir: path.join("/tmp/genesis-state", "devices"),
      pendingPath: path.join("/tmp/genesis-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/genesis-state", "devices", "paired.json"),
    });
  });
