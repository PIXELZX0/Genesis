...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", () => {
    ...
    withEnv(
      {
        ...
        GENESIS_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "GENESIS_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: GENESIS_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        GENESIS_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "GENESIS_BLANK_ENV",
