...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredGenesisTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_GENESIS_TMP_DIR) {
...
describe("resolvePreferredGenesisTmpDir", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/genesis when /tmp/genesis is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/genesis when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/genesis exists but is not writable",
    ...
    {
      name: "falls back when /tmp/genesis is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/genesis is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/genesis is group/other writable",
  ...
  it("repairs existing /tmp/genesis permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_GENESIS_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_GENESIS_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_GENESIS_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/genesis after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_GENESIS_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_GENESIS_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_GENESIS_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_GENESIS_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Genesis temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "genesis");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredGenesisTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_GENESIS_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Genesis temp dir/);
  });
});
