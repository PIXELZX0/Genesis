...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers GENESIS_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for GENESIS_HOME",
      env: { GENESIS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when GENESIS_HOME is tilde-only and no fallback home exists",
      env: { GENESIS_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores GENESIS_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          GENESIS_HOME: "/srv/genesis-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/genesis-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/genesis-home " },
      expected: "/srv/genesis-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/genesis-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of GENESIS_HOME", () => {
    expect(
