...
function setPackageRoot(root: string, name = "genesis") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./genesis-root.js").resolveGenesisPackageRootSync,
  asyncResolver: typeof import("./genesis-root.js").resolveGenesisPackageRoot,
  opts: Parameters<typeof import("./genesis-root.js").resolveGenesisPackageRootSync>[0],
...
vi.mock("./genesis-root.fs.runtime.js", () => ({
  genesisRootFsSync: mockFsModule(),
  genesisRootFs: mockFsPromisesModule(),
}));
...
describe("resolveGenesisPackageRoot", () => {
  ...
  it.each([
    ...
    {
      name: "falls through from a non-genesis moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-genesis");
    ...
    },
    {
      ...
      setup: () => {
        ...
        setPackageRoot(pkgRoot, "not-genesis");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "genesis.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "genesis");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
