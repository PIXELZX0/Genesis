...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.GENESIS_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-current-bindings-"));
    process.env.GENESIS_STATE_DIR = testStateDir;
