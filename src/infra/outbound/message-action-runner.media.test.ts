...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Genesis tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideGenesis = path.join(os.tmpdir(), "outside-genesis", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
