...
describe("tmp-genesis-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__genesisTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_GENESIS_TMP_DIR, resolvePreferredGenesisTmpDir } from "./src/infra/tmp-genesis-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_GENESIS_TMP_DIR,
            resolverType: typeof resolvePreferredGenesisTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-genesis-dir-browser-entry.ts",
      },
      write: false,
    });
