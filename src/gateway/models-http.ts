...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([GENESIS_MODEL_ID, GENESIS_DEFAULT_MODEL_ID]);
  ids.add(`genesis/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`genesis/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== GENESIS_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
