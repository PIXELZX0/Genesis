...
import { teardownEmbeddedForConn } from "../mcp-oauth-embedded.js";
...
export function attachGatewayWsConnectionHandler(params: AttachGatewayWsConnectionHandlerParams) {
  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    const preauthBudgetKey = (
      socket as WebSocket & {
        __genesisPreauthBudgetClaimed?: boolean;
        __genesisPreauthBudgetKey?: string;
      }
    ).__genesisPreauthBudgetKey;
    (
      socket as WebSocket & {
        __genesisPreauthBudgetClaimed?: boolean;
      }
    ).__genesisPreauthBudgetClaimed = true;
    ...
    socket.once("close", (code, reason) => {
      ...
      teardownEmbeddedForConn(connId);
