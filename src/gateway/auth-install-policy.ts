...
type GatewayInstallAuthMode = NonNullable<NonNullable<GenesisConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: GenesisConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.GENESIS_GATEWAY_PASSWORD) ||
