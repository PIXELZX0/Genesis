...
function getLastPluginLoadLogger(): {
  ...
  const call = loadGenesisPlugins.mock.calls.at(-1)?.[0] as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadGenesisPlugins.mockReturnValue(createRegistry([]));
...
describe("loadGatewayPlugins", () => {
  ...
  test("loads only gateway startup plugin ids", async () => {
    ...
    expect(loadGenesisPlugins).toHaveBeenCalledWith(
  ...
  test("pins the initial startup channel registry against later active-registry churn", async () => {
    ...
    loadGenesisPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", async () => {
    ...
    expect(loadGenesisPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://genesis.pixelzx.com/docs/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadGenesisPlugins.mockReturnValue(pluginRegistry);
