...
import { clearRuntimeConfigSnapshot, type GenesisConfig } from "../config/config.js";
...
const CLI_DEBUG = isTruthyEnvValue(process.env.GENESIS_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.GENESIS_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.GENESIS_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "GENESIS_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.GENESIS_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const cliImageArg =
        process.env.GENESIS_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.GENESIS_LIVE_CLI_BACKEND_IMAGE_MODE) ??
      ...
      const cfgWithCliBackends = cfg as GenesisConfig & {
        agents?: {
          defaults?: {
