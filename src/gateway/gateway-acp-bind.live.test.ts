...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.GENESIS_LIVE_ACP_BIND);
...
function resolveLiveParentModel(): string {
  return normalizeOpenAiModelRef(
    process.env.GENESIS_LIVE_ACP_BIND_PARENT_MODEL?.trim() ||
      process.env.GENESIS_LIVE_ACP_BIND_CODEX_MODEL?.trim() ||
...
async function prepareCodexHomeForLiveBindTest(): Promise<void> {
  ...
  const model = process.env.GENESIS_LIVE_ACP_BIND_CODEX_MODEL?.trim() || DEFAULT_LIVE_CODEX_MODEL;
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.GENESIS_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.GENESIS_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.GENESIS_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "genesis.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.GENESIS_GATEWAY_PORT = previous.port;
        }
