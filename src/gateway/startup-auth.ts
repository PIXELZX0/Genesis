...
import type { GatewayAuthConfig, GatewayTailscaleConfig, GenesisConfig } from "../config/config.js";
...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken = trimToUndefined(params.env.GENESIS_GATEWAY_TOKEN);
  if (envToken) {
    return true;
  }
  if (
