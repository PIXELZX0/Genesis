...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.GENESIS_CONFIG_PATH) {
    configPaths.add(process.env.GENESIS_CONFIG_PATH);
  }
  if (process.env.GENESIS_STATE_DIR) {
    configPaths.add(path.join(process.env.GENESIS_STATE_DIR, "genesis.json"));
  }
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".genesis-test-suite");
  ...
  tempControlUiRoot = path.join(tempHome, ".genesis-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>genesis-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.GENESIS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
