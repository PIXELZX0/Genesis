...
let lastCreateGenesisToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/genesis-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateGenesisToolsContext?.agentTo,
          agentThreadId: lastCreateGenesisToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createGenesisTools: (ctx: Record<string, unknown>) => {
      lastCreateGenesisToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateGenesisToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-genesis-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-genesis-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateGenesisToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateGenesisToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateGenesisToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateGenesisToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateGenesisToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateGenesisToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-genesis-message-to": "channel:24514",
        "x-genesis-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
