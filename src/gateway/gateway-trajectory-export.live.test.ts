...
const CODEX_HARNESS_LIVE = process.env.GENESIS_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.GENESIS_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["GENESIS_TRAJECTORY", "GENESIS_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-genesis-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.GENESIS_TRAJECTORY = "1";
      process.env.GENESIS_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".genesis", "trajectory-exports", "bundle");
