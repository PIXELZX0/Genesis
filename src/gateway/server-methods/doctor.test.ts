...
const loadConfig = vi.hoisted(() => vi.fn(() => ({}) as GenesisConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: GenesisConfig, _agentId: string) => "/tmp/genesis"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: GenesisConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/genesis");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: GenesisConfig, agentId: string) => {
      if (agentId === "alpha") {
  ...
  it("reads dreaming config from the selected memory slot plugin", async () => {
    loadConfig.mockReturnValue({
      plugins: {
        slots: {
          memory: "memos-local-genesis-plugin",
        },
        entries: {
          "memos-local-genesis-plugin": {
            config: {
              dreaming: {
                enabled: true,
                frequency: "0 */4 * * *",
  ...
  it("merges workspace store errors when multiple workspace stores are unreadable", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: GenesisConfig, agentId: string) =>
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/genesis");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/genesis/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/genesis/.genesis-repair/dreaming/2026-04-11T22-00-00-000Z",
  ...
  it("dedupes exact dream diary duplicates for control-ui callers", async () => {
    ...
    dedupeDreamDiaryEntries.mockResolvedValue({
      dreamsPath: "/tmp/genesis/DREAMS.md",
      removed: 2,
      kept: 7,
    });
    const respond = vi.fn();
