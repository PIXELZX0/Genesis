...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.GENESIS_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const genesisDir = path.join(params.homeDir, ".genesis");
  await fs.mkdir(genesisDir, { recursive: true });
  await fs.writeFile(
    path.join(genesisDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.GENESIS_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-live-codex-bind-"));
      const tempHome = path.join(tempRoot, "home");
      const stateDir = path.join(tempRoot, "state");
      const workspace = path.join(tempRoot, "workspace");
      const configPath = path.join(tempRoot, "genesis.json");
      ...
      const bindModel =
        process.env.GENESIS_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("GENESIS_CONFIG_PATH", previous.configPath);
        restoreEnvVar("GENESIS_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("GENESIS_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("GENESIS_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("GENESIS_SKIP_CRON", previous.skipCron);
        restoreEnvVar("GENESIS_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("GENESIS_STATE_DIR", previous.stateDir);
      }
    },
