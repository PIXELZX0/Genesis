...
beforeAll(async () => {
  previousMinimalGateway = process.env.GENESIS_TEST_MINIMAL_GATEWAY;
...
afterAll(async () => {
  ...
  if (previousMinimalGateway === undefined) {
    ...
    process.env.GENESIS_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
  }
});
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-session-message-"));
