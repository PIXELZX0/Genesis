...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "genesis-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".genesis", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["GENESIS_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  process.env.GENESIS_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "genesis-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".genesis", "genesis.json");
      const bundledPluginsDir = path.join(tempHome, "genesis-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.GENESIS_STATE_DIR = path.join(tempHome, ".genesis");
      process.env.GENESIS_CONFIG_PATH = configPath;
      process.env.GENESIS_SKIP_CHANNELS = "1";
