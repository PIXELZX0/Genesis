...
function gatewayTokenConfig(config: GenesisConfig): GenesisConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: GenesisConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/genesis-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: GenesisConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.GENESIS_SKIP_CHANNELS;
  const previousSkipProviders = process.env.GENESIS_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.GENESIS_SKIP_CHANNELS;
    } else {
      process.env.GENESIS_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.GENESIS_SKIP_PROVIDERS;
    } else {
      process.env.GENESIS_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
