...
import { ensureGenesisExecMarkerOnProcess } from "./infra/genesis-exec-env.js";
...
if (
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`[genesis] ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (containerTargetName && parsed.profile) {
      console.error("[genesis] --container cannot be combined with --profile/--dev");
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[genesis] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
