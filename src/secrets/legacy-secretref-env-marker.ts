...
function toCandidate(
  ...
  defaults: NonNullable<GenesisConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: GenesisConfig): {
  config: GenesisConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as GenesisConfig & Record<string, unknown>;
