...
function getSecretProviders(config: GenesisConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: GenesisConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: GenesisConfig): Array<{ value: SecretRefSource; label: string }> {
...
function resolveConfigureAgentId(config: GenesisConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: GenesisConfig): Promise<void> {
  while (true) {
  ...
  if (!params.providersOnly) {
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "genesis.json",
