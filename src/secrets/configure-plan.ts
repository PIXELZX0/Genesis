...
export function buildConfigureCandidates(config: GenesisConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredGenesisConfig?: GenesisConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
  ...
  const genesisCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `genesis.json` as const,
  ...
  return [...genesisCandidates, ...authCandidates].toSorted((a, b) =>
