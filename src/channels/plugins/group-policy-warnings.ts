...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: GenesisConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: GenesisConfig }>(
  collector: WarningCollector<{ cfg: GenesisConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: GenesisConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: GenesisConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: GenesisConfig },
>(
  projectCfg: (cfg: GenesisConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: GenesisConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: GenesisConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: GenesisConfig },
>(params: {
  providerConfigPresent: (cfg: GenesisConfig) => boolean;
