...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): GenesisConfig {
  if (accounts === null) {
    return {
      channels: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: GenesisConfig) => string[];
  input: GenesisConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: GenesisConfig, expected: string) {
