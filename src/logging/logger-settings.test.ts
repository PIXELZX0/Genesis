...
let originalGenesisLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.GENESIS_TEST_FILE_LOG;
  originalGenesisLogLevel = process.env.GENESIS_LOG_LEVEL;
  delete process.env.GENESIS_TEST_FILE_LOG;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.GENESIS_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalGenesisLogLevel === undefined) {
    delete process.env.GENESIS_LOG_LEVEL;
  } else {
    process.env.GENESIS_LOG_LEVEL = originalGenesisLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.GENESIS_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
