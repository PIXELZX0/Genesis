...
export function listExplicitConfiguredChannelIdsForConfig(config: GenesisConfig): string[] {
...
function listDisabledChannelIdsForConfig(config: GenesisConfig): string[] {
  const channels = config.channels;
  if (!channels || typeof channels !== "object" || Array.isArray(channels)) {
    return [];
  }
