...
const suiteTempRootTracker = createSuiteTempRootTracker("genesis-plugin-install-npm-spec");
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@genesis/voice-call@0.0.2-beta.1",
      name: "@genesis/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@genesis/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@genesis/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls as Array<[unknown, unknown]>,
        expectedSpec: "@genesis/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
