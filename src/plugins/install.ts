...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_GENESIS_EXTENSIONS: "missing_genesis_extensions",
  EMPTY_GENESIS_EXTENSIONS: "empty_genesis_extensions",
...
function ensureGenesisExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_GENESIS_EXTENSIONS,
    };
  }
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "genesis security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureGenesisExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function linkGenesisPeerDependencies(params: {
  ...
  const hostRoot = resolveGenesisPackageRootSync({
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureGenesisExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from genesis.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-genesis"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json genesis.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Genesis >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set GENESIS_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Genesis >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Genesis and retry.`,
  ...
  return await installPluginDirectoryIntoExtensions({
    ...
    afterInstall: async (installedDir) => {
      ...
      // outside the install root — a rule our trusted host-genesis link
      ...
      await linkGenesisPeerDependencies({
        installedDir,
