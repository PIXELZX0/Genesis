...
function hasTrustedGenesisRootIndicator(params: {
  packageRoot: string;
  ...
  const hasGenesisBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("genesis")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.genesis === "string");
  const hasGenesisEntrypoint = fs.existsSync(path.join(params.packageRoot, "genesis.mjs"));
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedGenesisRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedGenesisRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedGenesisRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveGenesisPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedGenesisRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.GENESIS_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `genesis/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.genesisPackageRoot, "extensions"),
    path.join(params.genesisPackageRoot, "dist", "extensions"),
    path.join(params.genesisPackageRoot, "dist-runtime", "extensions"),
