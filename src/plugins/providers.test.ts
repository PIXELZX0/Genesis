...
type LoadGenesisPlugins = typeof import("./loader.js").loadGenesisPlugins;
...
const loadGenesisPluginsMock = vi.fn<LoadGenesisPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/genesis.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadGenesisPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: GenesisConfig = {
    plugins: {},
  };
  const autoEnabledConfig: GenesisConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadGenesisPlugins: (...args: Parameters<LoadGenesisPlugins>) =>
        loadGenesisPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadGenesisPluginsMock.mockReset();
