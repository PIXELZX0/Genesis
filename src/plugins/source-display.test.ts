...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "genesis", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".genesis", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".genesis", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "genesis-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".genesis", "extensions"),
      },
    });
  });
});
