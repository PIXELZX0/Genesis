    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/firecrawl/genesis.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
describe("resolvePluginWebFetchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadGenesisPluginsMock = vi.spyOn(loaderModule, "loadGenesisPlugins").mockImplementation(() => {
  ...
  it("does not force a fresh snapshot load when the same web-provider load is already in flight", () => {
    ...
    loadGenesisPluginsMock.mockImplementation(() => {
