...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Genesis uses this to expose
 ...
 *
 * Runs after Genesis resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Genesis auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Genesis wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Genesis builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Genesis no
...
/**
 * Provider-owned stream wrapper hook after Genesis applies its generic
 ...
 *
 * Return `true` or `false` to override Genesis's built-in provider cache TTL
 ...
 *
 * Runs only after Genesis exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Genesis should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Genesis loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Genesis falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Genesis calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Genesis performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Genesis registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Genesis wrappers.
   *
   ...
   *
   * Called after Genesis resolves the raw configured credential but before the
   ...
   * hint than Genesis's generic auth-store guidance.
   */
   ...
   * Genesis's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Genesis should surface a provider-specific hint for
   ...
   * merging. Genesis deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Genesis stores the
   ...
   * without replacing the full Genesis-owned system prompt.
   */
   ...
   *
   * Runs after Genesis's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Genesis's default fallback policy.
   */
   ...
   * rewrites after Genesis has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Genesis handles
   ...
   *
   * Genesis uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `genesis doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `genesis doctor` should offer.
   *
   ...
   *
   * Genesis calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Genesis's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Genesis falls back to that stored profile.
   */
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Genesis session when available. */
  sessionFile?: string;
...
export type GenesisPluginHttpRouteAuth = "gateway" | "plugin";
export type GenesisPluginHttpRouteMatch = "exact" | "prefix";
export type GenesisPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";
...
export type GenesisPluginHttpRouteParams = {
  path: string;
 ...
 * If you want Genesis to keep a plugin command lazy-loaded while still
 ...
 */
export type GenesisPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type GenesisPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type GenesisPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type GenesisPluginSecurityAuditContext = {
  config: GenesisConfig;
...
export type GenesisPluginSecurityAuditCollector = (
  ctx: GenesisPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type GenesisGatewayDiscoveryAdvertiseContext = {
...
export type GenesisGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: GenesisGatewayDiscoveryAdvertiseContext,
...
export type GenesisPluginDefinition = {
  ...
  reload?: GenesisPluginReloadRegistration;
  nodeHostCommands?: GenesisPluginNodeHostCommand[];
  securityAuditCollectors?: GenesisPluginSecurityAuditCollector[];
  register?: (api: GenesisPluginApi) => void;
  activate?: (api: GenesisPluginApi) => void;
};

export type GenesisPluginModule = GenesisPluginDefinition | ((api: GenesisPluginApi) => void);
...
export type PluginConfigMigration = (config: GenesisConfig) =>
  | {
      config: GenesisConfig;
      changes: string[];
    }
...
export type GenesisPluginApi = {
  ...
  registerHttpRoute: (params: GenesisPluginHttpRouteParams) => void;
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Genesis can keep
  ...
  registerReload: (registration: GenesisPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: GenesisPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: GenesisPluginSecurityAuditCollector) => void;
  registerService: (service: GenesisPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: GenesisGatewayDiscoveryService) => void;
