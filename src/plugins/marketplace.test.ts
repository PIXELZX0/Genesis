...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("genesis-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("genesis-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("genesis-marketplace-test-", async (homeDir) => {
      const genesisHome = path.join(homeDir, "genesis-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, GENESIS_HOME: genesisHome },
