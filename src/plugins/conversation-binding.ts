...
const APPROVALS_PATH = "~/.genesis/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "genesis-app-server:thread:",
  "genesis-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("genesis.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("genesis.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Genesis instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
