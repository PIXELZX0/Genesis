...
function readDevicePairPublicUrlFromConfig(cfg: GenesisConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.GENESIS_GATEWAY_PASSWORD)) {
    return false;
  }
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: GenesisConfig): Promise<void> {
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the Genesis mobile app (Onboarding -> Scan QR).",
          "",
        ];
