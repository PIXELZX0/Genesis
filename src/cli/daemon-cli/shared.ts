...
export function renderRuntimeHints(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.GENESIS_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.GENESIS_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const hints = buildPlatformServiceStartHints({
    installCommand: formatCliCommand("genesis gateway install", env),
    startCommand: formatCliCommand("genesis gateway", env),
