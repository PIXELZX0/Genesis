...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("genesis gateway restart")} and re-check with ${formatCliCommand("genesis gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.GENESIS_PROFILE);
      defaultRuntime.error(
