...
function appendClawHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`genesis skills search\`, \`genesis skills install\`, and \`genesis skills update\` for ClawHub-backed skills.`;
}
...
export function formatSkillInfo(
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`genesis config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$GENESIS_CONFIG_PATH")} ${theme.muted("(default: ~/.genesis/genesis.json)")}`,
    );
  }
