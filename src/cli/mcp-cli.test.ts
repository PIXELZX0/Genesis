...
const serveGenesisChannelMcp = mocks.serveGenesisChannelMcp;
...
async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("genesis-cli-mcp-home-", async () => {
      ...
      expect(serveGenesisChannelMcp).toHaveBeenCalledWith({
