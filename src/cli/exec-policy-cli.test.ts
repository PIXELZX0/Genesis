...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: GenesisConfig) => void }) => {
      ...
      return {
        ...
        previousHash: "hash-1",
        snapshot: { path: "/tmp/genesis.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: GenesisConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: GenesisConfig }>
    >(async () => ({
      path: "/tmp/genesis.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: GenesisConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/genesis.json");
    ...
    expect(output).not.toContain("/tmp/genesis.json\nforged");
