...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "genesis.pixelzx.com/docs/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($GENESIS_CONFIG_PATH, default ~/.genesis/genesis.json):",
        ),
      );
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active Genesis config ($GENESIS_CONFIG_PATH, default ~/.genesis/genesis.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
