...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/genesis-root")).toEqual([
      path.join("/tmp/genesis-root", "dist", "index.js"),
      path.join("/tmp/genesis-root", "dist", "index.mjs"),
      path.join("/tmp/genesis-root", "dist", "entry.js"),
      path.join("/tmp/genesis-root", "dist", "entry.mjs"),
    ]);
  });
});
