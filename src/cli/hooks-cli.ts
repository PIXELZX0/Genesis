...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "genesis.pixelzx.com/docs/cli/hooks")}\n`,
    );
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `genesis plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`genesis hooks install` is deprecated; use `genesis plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `genesis plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`genesis hooks update` is deprecated; use `genesis plugins update`."),
      );
