...
export function isPrivateQaCliEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.GENESIS_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function resolvePrivateQaSourceModuleSpecifier(params?: {
  ...
  argv1?: string;
  moduleUrl?: string;
  resolvePackageRootSync?: typeof resolveGenesisPackageRootSync;
  ...
  const resolvePackageRootSync = params?.resolvePackageRootSync ?? resolveGenesisPackageRootSync;
...
export function loadPrivateQaCliModule(params?: {
  ...
  if (!specifier) {
    throw new Error("Private QA CLI is only available from an Genesis source checkout.");
  }
