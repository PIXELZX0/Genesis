...
import { hasUnknownRootOption } from "./argv.js";
...
export function shouldSkipPluginCommandRegistration(params: {
  ...
  if (!params.primary) {
    return (
      resolveCliArgvInvocation(params.argv).hasHelpOrVersion || hasUnknownRootOption(params.argv)
    );
  }
  return false;
}
...
export function shouldEagerRegisterSubcommands(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.GENESIS_DISABLE_LAZY_SUBCOMMANDS);
}
