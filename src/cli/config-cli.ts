...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "genesis config set secrets.providers.vault --provider-source file --provider-path /etc/genesis/secrets.json --provider-mode json",
);
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as GenesisConfig;
...
export function registerConfigCli(program: Command) {
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing genesis.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for genesis.json")
    .action(async () => {
