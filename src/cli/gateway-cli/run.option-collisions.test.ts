...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/genesis-stability.json",
  path: "/tmp/genesis-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/genesis-control-ui" as string | null,
}));
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/genesis-control-ui";
  ...
  it.each([
    ...
    delete process.env.GENESIS_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.GENESIS_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `genesis onboard --mode local` or `genesis setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("restores last-known-good config before startup when the effective config is invalid", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/genesis-test-missing-config.json",
      config: {},
      parsed: null,
    ...
    expect(gatewayLogMessages).toContain(
      "gateway: restored invalid effective config from last-known-good backup: /tmp/genesis-test-missing-config.json",
    );
    expect(startGatewayServer).toHaveBeenCalledWith(
      19170,
      expect.objectContaining({
