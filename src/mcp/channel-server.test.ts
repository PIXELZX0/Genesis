...
import { z } from "zod";
import { createGenesisChannelMcpServer, GenesisChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createGenesisChannelMcpServer({
...
describe("genesis channel mcp server", () => {
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new GenesisChannelBridge({} as never, {
