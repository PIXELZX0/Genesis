...
vi.mock("./channel-bridge.js", () => ({
  GenesisChannelBridge: class MockGenesisChannelBridge {
...
describe("serveGenesisChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveGenesisChannelMcp } = await import("./channel-server.js");

    const servePromise = serveGenesisChannelMcp({ verbose: false });
