...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `genesis-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.GENESIS_STATE_DIR ?? "/tmp/genesis",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.GENESIS_CONFIG_PATH ?? `${stateDir}/genesis.json`,
    );
    ...
    delete process.env.GENESIS_TUI_SETUP_AUTH_SOURCE;
  });
  ...
  it("falls back to GENESIS_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("keeps normal TUI local password mode env precedence by default", async () => {
    ...
    await withEnvAsync({ GENESIS_GATEWAY_PASSWORD: "env-password" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("uses configured local password for setup-launched TUI despite stale gateway password env", async () => {
    ...
    await withEnvAsync(
      {
        GENESIS_GATEWAY_PASSWORD: "stale-env-password", // pragma: allowlist secret
        GENESIS_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
        expect(result.password).toBe("config-password");
  ...
  it("still resolves env SecretRefs for setup-launched TUI config auth", async () => {
    loadConfig.mockReturnValue({
      ...
      gateway: {
        ...
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "GENESIS_GATEWAY_PASSWORD" },
        },
      },
    });
    ...
    await withEnvAsync(
      {
        GENESIS_GATEWAY_PASSWORD: "resolved-ref-password", // pragma: allowlist secret
        GENESIS_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
  ...
  it("prefers GENESIS_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "GENESIS_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-tui-file-secret-"));
