...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        GENESIS_BASE_ENV: "base",
        GENESIS_TO_REMOVE: undefined,
      },
      env: {
        GENESIS_TEST_ENV: "ok",
      },
    ...
    expect(resolved.GENESIS_BASE_ENV).toBe("base");
    expect(resolved.GENESIS_TEST_ENV).toBe("ok");
    expect(resolved.GENESIS_TO_REMOVE).toBeUndefined();
    expect(resolved.GENESIS_CLI).toBe(GENESIS_CLI_ENV_VALUE);
  });
