...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: genesis doctor --non-interactive"),
  ...
  triggerGenesisRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerGenesisRestart: mocks.triggerGenesisRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerGenesisRestart.mockReset();
    mocks.triggerGenesisRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerGenesisRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerGenesisRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerGenesisRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
