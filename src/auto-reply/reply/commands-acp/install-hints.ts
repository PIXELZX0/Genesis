...
export function resolveAcpInstallCommandHint(cfg: GenesisConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `genesis plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^genesis plugins install /u, "");
    ...
    return "genesis plugins install acpx";
  }
