...
const { createGenesisCodingToolsMock } = vi.hoisted(() => ({
  createGenesisCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createGenesisCodingTools: createGenesisCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createGenesisCodingToolsMock.mockClear();
    createGenesisCodingToolsMock.mockReturnValue([]);
  });

  it("opts command tool builds into gateway subagent binding", async () => {
    await resolveCommandsSystemPromptBundle(makeParams());

    expect(vi.mocked(createGenesisCodingTools)).toHaveBeenCalledWith(
