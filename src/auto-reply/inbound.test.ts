...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bgenesis\\b", "(invalid", "(a+)+$"] },
      },
    });
  ...
  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("gen\u200besis")).toBe("genesis");
  });
  ...
  it("strips safe mention patterns and ignores unsafe ones", () => {
    const stripped = stripMentions("genesis " + "a".repeat(28) + "!", {} as MsgContext, {
      messages: {
        groupChat: { mentionPatterns: ["\\bgenesis\\b", "(a+)+$"] },
      },
    });
