...
describe("Dockerfile", () => {
  it("uses shared multi-arch base image refs for all root Node stages", async () => {
    ...
    expect(dockerfile).toContain("FROM ${GENESIS_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${GENESIS_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${GENESIS_NODE_BOOKWORM_IMAGE} AS base-default");
    expect(dockerfile).toContain("FROM ${GENESIS_NODE_BOOKWORM_SLIM_IMAGE} AS base-slim");
  ...
  it("prunes runtime dependencies after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG GENESIS_EXTENSIONS");
    expect(dockerfile).toContain("ARG GENESIS_BUNDLED_PLUGIN_DIR");
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    ...
    expect(dockerfile).toContain(`ARG GENESIS_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bGENESIS_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for GENESIS_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
