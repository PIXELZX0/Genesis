...
type ConfiguredBinding = NonNullable<GenesisConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: GenesisConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/genesis" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
