...
const createTempDir = () => tempDirs.make("genesis-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets GENESIS_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "genesis",
    });

    expect(env.GENESIS_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("genesis");
  });

  it("overrides pre-existing GENESIS_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      GENESIS_SHELL: "wrong",
    });
    expect(env.GENESIS_SHELL).toBe("acp-client");
  ...
  it("preserves GENESIS_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        GENESIS_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["GENESIS_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Genesis bridge", () => {
    ...
    expect(env.GENESIS_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.GENESIS_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("GENESIS_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "genesis", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "genesis", "dist", "entry.js");
    const shimPath = path.join(dir, "genesis.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\genesis\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/genesis-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/genesis-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/genesis-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
