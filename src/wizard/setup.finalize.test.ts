...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: GenesisConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: GenesisConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: GenesisConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): GenesisConfig {
  return {
    tools: {
      web: {
        search: {
