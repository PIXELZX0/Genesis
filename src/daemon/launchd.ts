...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  ...
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.GENESIS_PROFILE));
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.GENESIS_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://genesis.pixelzx.com/docs/gateway",
    ].join("\n"),
  );
}
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "genesis gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "genesis gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `genesis gateway restart` is an explicit operator request to bring the
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "genesis gateway restart",
  });
