...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `genesis-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      GENESIS_LAUNCHD_LABEL: `ai.genesis.launchd-int-${testId}`,
      GENESIS_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
