...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds GENESIS_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `genesis gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.GENESIS_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.GENESIS_GATEWAY_TOKEN);
}
