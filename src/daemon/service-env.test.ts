...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.GENESIS_WINDOWS_TASK_NAME).toBe("Genesis Gateway");
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.GENESIS_WINDOWS_TASK_NAME).toBe("Genesis Gateway (work)");
    if (process.platform === "darwin") {
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through GENESIS_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", GENESIS_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.GENESIS_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through GENESIS_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", GENESIS_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.GENESIS_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits GENESIS_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
