...
const fixtureRoot = path.join(process.cwd(), ".tmp", `genesis-hook-install-${randomUUID()}`);
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "genesis.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "genesis.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@genesis/test-hooks@0.0.2-beta.1",
      name: "@genesis/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@genesis/test-hooks@beta"');
    }
  });
});
