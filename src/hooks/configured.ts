...
export function hasConfiguredInternalHooks(config: GenesisConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: GenesisConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
