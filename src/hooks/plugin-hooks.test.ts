...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "genesis-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.GENESIS_BUNDLED_HOOKS_DIR;
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.GENESIS_BUNDLED_HOOKS_DIR;
    } else {
      process.env.GENESIS_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".genesis", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): GenesisConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("genesis-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Genesis hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".genesis", "extensions", "claude-bundle");
