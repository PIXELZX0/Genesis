...
describe("loader", () => {
  ...
  describe("loadInternalHooks", () => {
    it("detects configured internal hook surfaces", () => {
      expect(hasConfiguredInternalHooks({} satisfies GenesisConfig)).toBe(false);
      expect(
    ...
    const expectNoCommandHookRegistration = async (cfg: GenesisConfig) => {
    ...
    it("skips hook discovery until internal hooks are configured", async () => {
      for (const cfg of [
        {} satisfies GenesisConfig,
        { hooks: {} } satisfies GenesisConfig,
        { hooks: { internal: {} } } satisfies GenesisConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
