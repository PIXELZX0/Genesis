...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "genesis-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "genesis-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "genesis-bundled");
      const plugin = makeHookEntry("shared", "genesis-plugin");
      const managed = makeHookEntry("shared", "genesis-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("genesis-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "genesis-managed");
      const workspace = makeHookEntry("shared", "genesis-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "genesis-workspace");
      const second = makeHookEntry("shared", "genesis-workspace");
      second.hook.handlerPath = "/tmp/genesis-workspace/shared/handler-2.js";
