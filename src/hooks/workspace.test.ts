...
function setupHardlinkHookWorkspace(hookName: string): {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-hooks-workspace-hardlink-"));
...
describe("hooks workspace", () => {
  ...
  it("does not let workspace hooks override managed hooks with the same name", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-hooks-collision-"));
    ...
    expect(entries[0]?.hook.source).toBe("genesis-managed");
  });

  it("treats configured extraDirs as managed hook sources", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-hooks-extra-"));
    const workspaceDir = path.join(root, "workspace");
    ...
    fs.writeFileSync(
      ...
      ["---", "name: shared-hook", 'metadata: {"genesis":{"events":["command:new"]}}', "---"].join(
        "\n",
      ),
    );
