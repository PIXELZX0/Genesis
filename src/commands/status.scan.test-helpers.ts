...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/genesis-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = GenesisConfig>(
  overrides: T = {} as T,
): GenesisConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): GenesisConfig {
