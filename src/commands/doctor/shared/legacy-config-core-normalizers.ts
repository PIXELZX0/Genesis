...
export function normalizeLegacyCommandsConfig(
  ...
  return {
    ...cfg,
    commands: commands as GenesisConfig["commands"],
  };
}

export function normalizeLegacyBrowserConfig(cfg: GenesisConfig, changes: string[]): GenesisConfig {
  ...
  return {
    ...cfg,
    browser: browser as GenesisConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as GenesisConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<GenesisConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<GenesisConfig["models"]>>;
type ModelDefinitionEntry = NonNullable<ModelProviderEntry["models"]>[number];
type AgentEmbeddedHarnessPatch = NonNullable<
  NonNullable<NonNullable<GenesisConfig["agents"]>["defaults"]>["embeddedHarness"]
>;
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  return changed
    ? {
        ...cfg,
        agents: nextAgents as GenesisConfig["agents"],
      }
    : cfg;
}
...
export function normalizeLegacyOpenAICodexModelsAddMetadata(
  ...
  return {
    ...
    models: {
      ...rawModels,
      providers: nextProviders as NonNullable<GenesisConfig["models"]>["providers"],
    },
  };
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<GenesisConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as GenesisConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<GenesisConfig["tools"]>["media"],
    },
  };
}
