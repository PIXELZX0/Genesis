...
export function normalizeLegacyTalkConfig(cfg: GenesisConfig, changes: string[]): GenesisConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as GenesisConfig["talk"]) ?? {};
