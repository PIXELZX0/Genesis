...
vi.mock("../agents/agent-paths.js", () => ({
  resolveGenesisAgentDir: () => process.env.GENESIS_AGENT_DIR ?? "/tmp/genesis-agent",
}));

vi.mock("../agents/agent-scope.js", () => ({
  resolveDefaultAgentId: () => "main",
  resolveAgentDir: (_config: unknown, agentId: string) => `/tmp/genesis-agents/${agentId}`,
  resolveAgentWorkspaceDir: (_config: unknown, agentId: string) =>
    `/tmp/genesis-workspaces/${agentId}`,
}));

vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/genesis-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.GENESIS_AGENT_DIR || "__main__";
}
...
async function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<GenesisConfig>;
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as GenesisConfig } : {}),
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as GenesisConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(requireGenesisAgentDir());
  }
  ...
  beforeAll(async () => {
    authTestRoot = (await setupAuthTestEnv("genesis-auth-")).stateDir;
