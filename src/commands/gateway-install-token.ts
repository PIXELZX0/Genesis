...
async function maybePersistAutoGeneratedGatewayInstallToken(params: {
  ...
  try {
    ...
    if (snapshot.exists && !snapshot.valid) {
      ...
      return { token: params.token, persisted: false };
    }
    ...
    if (!existingTokenRef && !baseConfigToken) {
      await writeConfigFile(
        {
          ...
          gateway: {
            ...baseConfig.gateway,
            mode: baseConfig.gateway?.mode ?? "local",
            auth: {
      ...
      return { token: params.token, persisted: true };
    }
    if (baseConfigToken) {
      return { token: baseConfigToken, persisted: false };
    }
    params.warnings.push(
      "Warning: gateway.auth.token is SecretRef-managed; skipping plaintext token persistence.",
    );
    return { token: undefined, persisted: false };
  } catch (err) {
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("genesis config set gateway.auth.mode token")} or ${formatCliCommand("genesis config set gateway.auth.mode password")}.`,
  ].join(" ");
}
...
export async function resolveGatewayInstallToken(
  ...
  let generatedTokenPersisted = false;
  if (!token && !tokenRefConfigured && allowAutoGenerate) {
    ...
    if (persistGeneratedToken) {
      const persisted = await maybePersistAutoGeneratedGatewayInstallToken({
        token,
        config: cfg,
      ...
      generatedTokenPersisted = persisted.persisted;
    }
  }
  ...
  return {
    ...
    unavailableReason,
    warnings,
    ...(generatedTokenPersisted ? { generatedTokenPersisted } : {}),
  };
}
