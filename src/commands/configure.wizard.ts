...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.GENESIS_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.GENESIS_GATEWAY_PASSWORD ?? configuredPassword;
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<GenesisConfig["tools"]>["web"]>["search"];
