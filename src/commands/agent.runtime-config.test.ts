...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: GenesisConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => GenesisConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: GenesisConfig, sourceConfig: GenesisConfig) => void>(),
);
...
function mockConfig(home: string, storePath: string): GenesisConfig {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteMock.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as GenesisConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand runtime config", () => {
  ...
  it("includes channel secret targets when delivery is requested", async () => {
    await withTempHome(async (home) => {
      ...
      } as unknown as GenesisConfig["channels"];
      resolveCommandConfigWithSecretsMock.mockResolvedValueOnce({
