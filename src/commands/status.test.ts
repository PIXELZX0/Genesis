...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByGenesis: Boolean(command),
...
async function createMockStatusScanResult(params: { includePluginCompatibility?: boolean } = {}) {
  ...
  return {
    ...
    gatewayProbeAuth: process.env.GENESIS_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.genesis.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.genesis.node.plist",
    }),
  }),
}));
...
vi.mock("../infra/genesis-root.js", () => ({
  ...
  resolveGenesisPackageRootSync: vi.fn(() => "/tmp/genesis"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("genesis status --all") ||
          line.includes("genesis --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("genesis --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("GENESIS_GATEWAY_TOKEN", "abcd1234", async () => {
