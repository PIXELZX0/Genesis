...
describe("doctor command", () => {
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("genesis config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain("genesis config set gateway.auth.mode password");
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.GENESIS_GATEWAY_TOKEN;
    delete process.env.GENESIS_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
