...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.GENESIS_GATEWAY_TOKEN);
  ...
  // plaintext > ambient GENESIS_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like GENESIS_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }
      ...
      // branch, an ambient GENESIS_GATEWAY_TOKEN (or randomToken() fallback)
