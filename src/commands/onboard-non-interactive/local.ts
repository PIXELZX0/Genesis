...
export async function runNonInteractiveLocalSetup(params: {
  ...
  let nextConfig: GenesisConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `genesis gateway run` or `genesis agent --local`.",
              ]
            : [`Run \`${formatCliCommand("genesis gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("genesis gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
