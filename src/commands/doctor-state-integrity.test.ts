...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("GENESIS_STATE_DIR is not set");
  }
...
async function runStateIntegrityText(cfg: GenesisConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.GENESIS_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it("prints genesis-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: GenesisConfig = {};
    ...
    expect(text).toMatch(/genesis sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/genesis sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /genesis sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
