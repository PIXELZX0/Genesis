...
function listOrphanAgentDirs(cfg: GenesisConfig, stateDir: string): OrphanAgentDir[] {
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for GENESIS_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // GENESIS_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldSuppressOrphanTranscriptWarning(cfg: GenesisConfig, agentId: string): boolean {
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.genesis).",
        `  Set locally: GENESIS_STATE_DIR=~/.genesis ${formatCliCommand("genesis doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`genesis sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`genesis sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`genesis sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
