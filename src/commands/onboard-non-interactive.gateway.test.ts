...
const testConfigStore = new Map<string, GenesisConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.GENESIS_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.GENESIS_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("GENESIS_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "genesis.json");
}
...
function readTestConfig<T = GenesisConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  replaceConfigFile: async ({ nextConfig }: { nextConfig: GenesisConfig }) => {
    testConfigStore.set(resolveTestConfigPath(), nextConfig);
  },
  resolveGatewayPort: (cfg: GenesisConfig) => cfg.gateway?.port ?? 18789,
}));
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "genesis"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `genesis gateway status --deep` for more detail.");
