...
function usesClaudeCliModelSelection(cfg: GenesisConfig): boolean {
...
function hasClaudeCliConfigSignals(cfg: GenesisConfig): boolean {
...
function resolveClaudeCliCommand(cfg: GenesisConfig): string {
...
function formatWorkspaceHealthLine(workspaceDir: string, health: ClaudeCliDirHealth): string {
  ...
  if (health === "missing") {
    return `- Workspace: ${display} (missing; Genesis will create it on first run).`;
  }
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- Genesis auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Genesis auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- Genesis auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
