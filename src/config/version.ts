...
export function normalizeGenesisVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseGenesisVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameGenesisStableFamily(
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseGenesisVersion(current);
  const parsedTouched = parseGenesisVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameGenesisStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareGenesisVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: GenesisVersion): number {
