...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.genesis) || !isRecord(entry.genesis.channel)) {
      continue;
    }
    const channel = entry.genesis.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: GenesisConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: GenesisConfig, pluginId: string) => boolean;
