...
describe("config io write prepare", () => {
  ...
  it("preserves authored source-only nested fields during partial writes", () => {
      ...
      nextConfig: {
        plugins: {
          ...
          installs: {
            "genesis-web-search": {
              source: "npm",
              spec: "@ollama/genesis-web-search@0.2.2",
              installPath: "/tmp/genesis-web-search",
              resolvedName: "@ollama/genesis-web-search",
              resolvedVersion: "0.2.2",
            },
    ...
    expect(persisted.plugins?.installs?.["genesis-web-search"]).toEqual({
      source: "npm",
      spec: "@ollama/genesis-web-search@0.2.2",
      installPath: "/tmp/genesis-web-search",
      resolvedName: "@ollama/genesis-web-search",
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("genesis config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('genesis config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/genesis.json",
        expectedConfigPath: "/tmp/genesis.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: GenesisConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
