...
function hasConfiguredEmbeddedHarnessRuntime(cfg: GenesisConfig, env: NodeJS.ProcessEnv): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: GenesisConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: GenesisConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: GenesisConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: GenesisConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: GenesisConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: GenesisConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: GenesisConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: GenesisConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: GenesisConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: GenesisConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: GenesisConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: GenesisConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithEntries(cfg: GenesisConfig): boolean {
  return Array.isArray(cfg.plugins?.allow) && cfg.plugins.allow.length > 0 && hasPluginEntries(cfg);
}

function hasConfiguredProviderModelOrHarness(cfg: GenesisConfig, env: NodeJS.ProcessEnv): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: GenesisConfig, env: NodeJS.ProcessEnv): boolean {
...
export function configMayNeedPluginAutoEnable(cfg: GenesisConfig, env: NodeJS.ProcessEnv): boolean {
  if (hasPluginAllowlistWithEntries(cfg)) {
    return true;
  }
  if (hasConfiguredPluginConfigEntry(cfg)) {
...
function isBuiltInChannelAlreadyEnabled(cfg: GenesisConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
