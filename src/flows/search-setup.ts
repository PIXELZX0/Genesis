...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<GenesisConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<GenesisConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: GenesisConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: GenesisConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: GenesisConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: GenesisConfig, result: GenesisConfig): GenesisConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as GenesisConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with genesis configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Genesis will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://genesis.pixelzx.com/docs/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Genesis will store a reference instead of the API key.",
