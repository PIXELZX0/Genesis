...
export type DoctorHealthFlowContext = {
  ...
  cfg: GenesisConfig;
  cfgForPersistence: GenesisConfig;
  sourceConfigValid: boolean;
  configPath: string;
...
export function resolveDoctorMode(cfg: GenesisConfig): DoctorFlowMode {
  ...
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("genesis config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("genesis config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("genesis doctor --fix")}" to apply changes.`);
  }
}
