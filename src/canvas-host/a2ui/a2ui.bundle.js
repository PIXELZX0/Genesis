...
var GenesisA2UIHost = class extends i$6 {
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["genesis:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["genesis:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		if (handler?.postMessage) try {
			if (handler === globalThis.genesisCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("genesis-a2ui-host")) customElements.define("genesis-a2ui-host", GenesisA2UIHost);
