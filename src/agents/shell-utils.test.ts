...
describe("detectRuntimeShell", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.GENESIS_SHELL;
...
describe("resolvePowerShellPath", () => {
  ...
  it("prefers PowerShell 7 in ProgramFiles", () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-pfiles-"));
    tempDirs.push(base);
  ...
  it("prefers ProgramW6432 PowerShell 7 when ProgramFiles lacks pwsh", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-pfiles-"));
    const programW6432 = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-pw6432-"));
  ...
  it("finds pwsh on PATH when not in standard install locations", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-pfiles-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-bin-"));
  ...
  it("falls back to Windows PowerShell 5.1 path when pwsh is unavailable", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-pfiles-"));
    const sysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-sysroot-"));
