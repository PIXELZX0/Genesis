...
describe("runCliAgent spawn path", () => {
  ...
  it("includes the Genesis skills prompt in CLI system prompts", () => {
  ...
  it("passes Genesis skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-cli-skills-"));
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("genesis-session");
  ...
  it("can preserve selected clearEnv keys for live CLI backend probes", async () => {
    try {
      process.env.GENESIS_LIVE_CLI_BACKEND_PRESERVE_ENV = '["SAFE_CLEAR"]';
      process.env.SAFE_CLEAR = "from-base";
      mockSuccessfulCliRun();
      await executePreparedCliRun(
        buildPreparedCliRunContext({
      ...
      delete process.env.GENESIS_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-cli-bootstrap-context-"));
