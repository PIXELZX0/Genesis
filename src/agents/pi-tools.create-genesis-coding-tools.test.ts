...
function expectNoSubagentControlTools(tools: ReturnType<typeof createGenesisCodingTools>) {
...
describe("createGenesisCodingTools", () => {
  const testConfig: GenesisConfig = {};

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createGenesisCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const codexTools = createGenesisCodingTools({
      config: testConfig,
    ...
    const disabledConfig: GenesisConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createGenesisCodingTools({
    ...
    const allowModelsConfig: GenesisConfig = {
      tools: {
        exec: {
    ...
    const oauthTools = createGenesisCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createGenesisCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(codingTools.some((tool) => tool.name === "message")).toBe(false);

    const cronTools = createGenesisCodingTools({
      config: { tools: { profile: "coding" } },
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createGenesisCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createGenesisCodingTools({
      modelProvider: "xai",
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "genesis-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "genesis-workspace"),
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-canonical-"));
    try {
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-legacy-alias-"));
    try {
      const tools = createGenesisCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
