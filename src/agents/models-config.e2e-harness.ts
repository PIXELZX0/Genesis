...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousGenesisAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousGenesisAgentDir = process.env.GENESIS_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousGenesisAgentDir === undefined) {
      delete process.env.GENESIS_AGENT_DIR;
    } else {
      process.env.GENESIS_AGENT_DIR = previousGenesisAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "GENESIS_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.GENESIS_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.GENESIS_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.GENESIS_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
