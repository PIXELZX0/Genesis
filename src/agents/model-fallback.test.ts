...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "genesis-auth-suite-mock");
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("genesis-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
