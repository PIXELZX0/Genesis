...
const planGenesisModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planGenesisModelsJson: (...args: unknown[]) => planGenesisModelsJsonMock(...args),
  }));
  ({ ensureGenesisModelsJson } = await import("./models-config.js"));
});

beforeEach(() => {
  planGenesisModelsJsonMock
    .mockReset()
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureGenesisModelsJson(first), ensureGenesisModelsJson(second)]);
      } finally {
