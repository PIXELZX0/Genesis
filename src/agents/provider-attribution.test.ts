...
describe("provider attribution", () => {
  it("resolves the canonical Genesis product and runtime version", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { GENESIS_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "genesis",
        version: "2026.3.22",
        "User-Agent": "genesis/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { GENESIS_VERSION: "2026.3.22" })).toEqual({
      originator: "genesis",
      version: "2026.3.22",
      "User-Agent": "genesis/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { GENESIS_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ GENESIS_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
