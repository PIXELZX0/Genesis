...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("genesis-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("genesis-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("genesis-tilde-test-mkdir-");
  ...
  it("ignores GENESIS_HOME for write operations", async () => {
    const genesisHome = await createTempDir("genesis-home-override-", os.tmpdir());
    const dir = await createTempDir("genesis-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("GENESIS_HOME", genesisHome);

    createHostWorkspaceWriteTool(genesisHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(genesisHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores GENESIS_HOME for mkdir operations", async () => {
    const genesisHome = await createTempDir("genesis-home-override-", os.tmpdir());
    const dir = await createTempDir("genesis-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(genesisHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores GENESIS_HOME for readFile operations", async () => {
    const genesisHome = await createTempDir("genesis-home-override-", os.tmpdir());
    const dir = await createTempDir("genesis-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(genesisHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores GENESIS_HOME for access operations", async () => {
    const genesisHome = await createTempDir("genesis-home-override-", os.tmpdir());
    const dir = await createTempDir("genesis-tilde-test-edit-");
