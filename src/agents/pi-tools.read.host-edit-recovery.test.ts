...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when GENESIS_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const genesisHome = path.join(tmpDir, "genesis-home");
    ...
    try {
      ...
      await expect(fs.access(path.join(genesisHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
