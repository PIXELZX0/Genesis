...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- Prefer the `canvas` tool with `action: "create"` to materialize inline HTML, a local path, or a URL as a hosted embed document before replying. `create` requires exactly one of `html`, `path`, or `url`; `kind` and `title` alone do not create content.',
    '- Use `canvas` with `action: "present"` and `id` only when you need to recover the embed preview for an existing hosted document. Do not use `snapshot` for hosted embed documents.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__genesis__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
