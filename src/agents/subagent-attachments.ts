...
function resolveAttachmentLimits(config: GenesisConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".genesis", "attachments");
  const relDir = path.posix.join(".genesis", "attachments", attachmentId);
