...
describe("ensureAgentWorkspace", () => {
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("genesis-workspace-");
    await fs.mkdir(path.join(tempDir, ".genesis"), { recursive: true });
    await fs.writeFile(
  ...
  it("does not seed HEARTBEAT.md into new workspaces", async () => {
    const tempDir = await makeTempWorkspace("genesis-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-workspace-hardlink-"));
    try {
