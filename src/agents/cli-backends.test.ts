...
function createClaudeCliOverrideConfig(config: CliBackendConfig): GenesisConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Genesis's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__genesis__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__genesis__*");
  ...
  it("keeps Claude permission mode unset when Genesis exec policy is not YOLO", () => {
    const resolved = resolveCliBackendConfig("claude-cli", {
