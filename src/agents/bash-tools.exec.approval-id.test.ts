...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".genesis", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-exec-approvals-"));
  });

  beforeEach(async () => {
    previousHome = process.env.HOME;
    previousUserProfile = process.env.USERPROFILE;
    previousBundledPluginsDir = process.env.GENESIS_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.GENESIS_DISABLE_BUNDLED_PLUGINS;
  ...
  afterEach(() => {
    ...
    if (previousDisableBundledPlugins === undefined) {
      ...
      process.env.GENESIS_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".genesis", "skills", "gog");
