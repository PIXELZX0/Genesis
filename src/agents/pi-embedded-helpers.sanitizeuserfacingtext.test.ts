...
describe("sanitizeUserFacingText", () => {
  ...
  it.each(["disk full", "ENOSPC: no space left on device"])(
    ...
    (input) => {
      expect(sanitizeUserFacingText(input, { errorContext: true })).toBe(
        "Genesis could not write local session data because the disk is full. Free some disk space and try again.",
      );
    },
  );
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `Genesis runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "Genesis runtime context (internal): is the label used by the old runtime block formatter.";
