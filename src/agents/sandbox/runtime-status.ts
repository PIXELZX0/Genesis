...
function resolveMainSessionKeyForSandbox(params: { cfg?: GenesisConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: GenesisConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  const explainCommand = runtime.sessionKey
    ? hasUnsafeControlChars(runtime.sessionKey)
      ? `genesis sandbox explain --agent ${runtime.agentId}`
      : `genesis sandbox explain --session ${shellEscapeSingleArg(runtime.sessionKey)}`
