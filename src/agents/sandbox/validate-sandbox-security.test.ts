...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when GENESIS_HOME points elsewhere", () => {
    vi.stubEnv("HOME", "/home/tester");
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "genesis-home-"));
...
describe("validateBindMounts", () => {
  ...
  it("allows legitimate project directory mounts", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-safe-"));
    expect(() =>
      validateBindMounts([
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "genesis-not-created");
    expect(() =>
  ...
  it("blocks bind sources outside allowed roots when allowlist is configured", () => {
    const allowedRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-allowed-root-"));
    const externalRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-external-"));
    expect(() =>
  ...
  it("allows bind sources in allowed roots when allowlist is configured", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-allowed-"));
    expect(() =>
  ...
  it("blocks reserved container target paths by default", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-reserved-default-"));
    expect(() =>
  ...
  it("allows reserved container target paths with explicit dangerous override", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-reserved-"));
    expect(() =>
...
describe("validateSandboxSecurity", () => {
  it("passes with safe config", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "genesis-sbx-safe-config-"));
    expect(() =>
      validateSandboxSecurity({
