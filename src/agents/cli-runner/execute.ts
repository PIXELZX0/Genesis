...
const CLI_BACKEND_PRESERVE_ENV = "GENESIS_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.GENESIS_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.GENESIS_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.GENESIS_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.GENESIS_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.GENESIS_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.GENESIS_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.GENESIS_MCP_TOKEN ||
            env.GENESIS_MCP_SESSION_KEY ||
            env.GENESIS_MCP_SENDER_IS_OWNER
          ) {
