...
const GENESIS_CLAUDE_PLUGIN_NAME = "genesis-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredGenesisTmpDir(), "genesis-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, GENESIS_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: GENESIS_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Genesis skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
