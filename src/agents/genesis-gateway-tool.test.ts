...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    try {
      await withEnvAsync({ GENESIS_STATE_DIR: stateDir, GENESIS_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/genesis-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/genesis-bin"] } } }',
      }),
    ).rejects.toThrow(
