...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/genesis/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./genesis-plugin-tools.js", () => ({
  resolveGenesisPluginToolsForOptions: () => [],
}));

vi.mock("./genesis-tools.nodes-workspace-guard.js", () => ({
...
describe("createGenesisTools TTS config wiring", () => {
  ...
  it("passes the resolved shared config into the tts tool", async () => {
    ...
    const { __testing, createGenesisTools } = await import("./genesis-tools.js");
