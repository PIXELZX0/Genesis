...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "genesis-adc-"));
...
function buildDemoLocalProviderCfg(apiKey: string): GenesisConfig {
  return {
    models: {
      providers: {
        "demo-local": {
