...
let createGenesisCodingTools: typeof import("./pi-tools.js").createGenesisCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      GENESIS_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "genesis-test-no-bundled-extensions"),
    },
    async () => {
      ({ createGenesisCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        GENESIS_SHELL_ENV_TIMEOUT_MS: "1",
        SHELL: "/bin/sh",
      },
      async () => {
        await run(ctx);
