...
let ensureGenesisModelsJsonMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ensureGenesisModelsJsonMock = vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false });
    vi.doMock("./models-config.js", () => ({
      ensureGenesisModelsJson: ensureGenesisModelsJsonMock,
    }));
  ...
  it("does not prepare models.json when loading catalog in read-only mode", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as GenesisConfig, readOnly: true });
