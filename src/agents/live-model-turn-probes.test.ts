...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "GENESIS_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { GENESIS_LIVE_MODEL_IMAGE_PROBE: "false" },
        "GENESIS_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { GENESIS_LIVE_MODEL_IMAGE_PROBE: "1" },
        "GENESIS_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
