...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `genesis security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as GenesisConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as GenesisConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isGenesisOwnerOnlyCoreToolName("gateway"),
    description:
