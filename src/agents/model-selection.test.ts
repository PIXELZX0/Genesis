...
function resolveAnthropicOpusThinking(cfg: GenesisConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: GenesisConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<GenesisConfig>) {
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as GenesisConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as GenesisConfig)).toBe(false);
    });
  });
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", async () => {
      const warnLogs = createWarnLogCapture("genesis-model-selection-test");
      try {
        const cfg: Partial<GenesisConfig> = {
          agents: {
            defaults: {
