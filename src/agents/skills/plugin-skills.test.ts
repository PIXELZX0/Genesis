    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "genesis.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "genesis.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "genesis.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  ...
  const acpxRoot = await tempDirs.make("genesis-acpx-plugin-");
  const helperRoot = await tempDirs.make("genesis-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("genesis-");
  const pluginRoot = await tempDirs.make("genesis-plugin-");
  const outsideDir = await tempDirs.make("genesis-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("genesis-");
    const pluginRoot = await tempDirs.make("genesis-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("genesis-");
    const pluginRoot = await tempDirs.make("genesis-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
