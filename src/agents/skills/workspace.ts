...
function resolveSkillsLimits(config?: GenesisConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "genesis-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "genesis-bundled") {
    return {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`genesis skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`genesis skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
