...
describe("env test utils", () => {
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "genesis-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousGenesisHome = process.env.GENESIS_HOME;
    const previousStateDir = process.env.GENESIS_STATE_DIR;
    const previousBundledDir = process.env.GENESIS_BUNDLED_PLUGINS_DIR;
    process.env.GENESIS_HOME = "/srv/genesis-home";
    process.env.GENESIS_STATE_DIR = "/srv/genesis-state";
    process.env.GENESIS_BUNDLED_PLUGINS_DIR = "/srv/genesis-bundled";
    ...
    try {
      ...
      expect(env.GENESIS_HOME).toBeUndefined();
      expect(env.GENESIS_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.GENESIS_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("GENESIS_HOME", previousGenesisHome);
      restoreEnvKey("GENESIS_STATE_DIR", previousStateDir);
      restoreEnvKey("GENESIS_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { GENESIS_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processGenesisHome: process.env.GENESIS_HOME,
          processBundledDir: process.env.GENESIS_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.GENESIS_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processGenesisHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.GENESIS_HOME).toBe("/srv/genesis-home");
    } finally {
      restoreEnvKey("GENESIS_HOME", previousGenesisHome);
    }
  });
