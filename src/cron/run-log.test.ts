...
describe("cron run log", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("genesis-cron-log-perms-", async (dir) => {
        const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("genesis-cron-log-dir-perms-", async (dir) => {
  ...
  it("does not include raw delivery targets in run-log search", async () => {
    await withRunLogDir("genesis-cron-log-target-query-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
      await fs.mkdir(path.dirname(logPath), { recursive: true });
      await fs.writeFile(
        logPath,
  ...
  it("read drains pending fire-and-forget writes", async () => {
    await withRunLogDir("genesis-cron-log-drain-", async (dir) => {
