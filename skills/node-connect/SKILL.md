---
name: node-connect
description: Diagnose Genesis Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Genesis is advertising that route, then fix pairing/auth.
...
- whether `genesis devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `genesis qr --json`. It uses the same setup-code payload Android scans.

```bash
genesis config get gateway.mode
genesis config get gateway.bind
genesis config get gateway.tailscale.mode
genesis config get gateway.remote.url
genesis config get gateway.auth.mode
genesis config get gateway.auth.allowTailscale
genesis config get plugins.entries.device-pair.config.publicUrl
genesis qr --json
genesis devices list
genesis nodes status
```

If this Genesis instance is pointed at a remote gateway, also run:

```bash
genesis qr --remote --json
```
...
If `genesis qr --json` says `Gateway is only bound to loopback`:
...
- `genesis devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run genesis qr again, rescan, then approve the pending device pairing.`
