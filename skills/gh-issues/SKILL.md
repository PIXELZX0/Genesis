...
```
CONFIG_PATH="${GENESIS_CONFIG_PATH:-${GENESIS_STATE_DIR:-$HOME/.genesis}/genesis.json}"
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Genesis dashboard or in the active Genesis config path (`$GENESIS_CONFIG_PATH`, default `~/.genesis/genesis.json`) under `skills.entries.gh-issues`."
...
CONFIG_PATH="${GENESIS_CONFIG_PATH:-${GENESIS_STATE_DIR:-$HOME/.genesis}/genesis.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/genesis.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export CONFIG_PATH="${GENESIS_CONFIG_PATH:-${GENESIS_STATE_DIR:-$HOME/.genesis}/genesis.json}"
