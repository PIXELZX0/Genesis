...
1. Download the runtime for your OS (extracts into `$GENESIS_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.genesis/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$GENESIS_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.genesis/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Genesis config file (`$GENESIS_CONFIG_PATH`, default `~/.genesis/genesis.json`):
