...
In the active Genesis config file (`$GENESIS_CONFIG_PATH`, default `~/.genesis/genesis.json`):
...
1. Check server bind: `CONFIG_PATH="${GENESIS_CONFIG_PATH:-${GENESIS_STATE_DIR:-$HOME/.genesis}/genesis.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
