...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.genesis) ? packageJson.genesis : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.genesis?.channel?.id) || left.name;
    const rightId = trimString(right.genesis?.channel?.id) || right.name;
