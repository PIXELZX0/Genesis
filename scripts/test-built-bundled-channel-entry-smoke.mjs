...
process.env.GENESIS_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "GENESIS_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    if (!packageJson.genesis?.channel) {
      continue;
    }
    ...
    const setupEntry = packageJson.genesis.setupEntry;
