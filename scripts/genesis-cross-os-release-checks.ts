...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.GENESIS_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.GENESIS_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.GENESIS_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const summary = {
    ...
    runnerOs: process.env.GENESIS_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.GENESIS_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        GENESIS_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `genesis-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".genesis");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    GENESIS_HOME: lane.homeDir,
    GENESIS_STATE_DIR: lane.stateDir,
    GENESIS_CONFIG_PATH: join(lane.stateDir, "genesis.json"),
    GENESIS_DISABLE_BONJOUR: "1",
    GENESIS_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.GENESIS_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'genesis.cmd'),
      (Join-Path $npmPrefix 'genesis.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command genesis -ErrorAction Stop
...
Write-Output "__GENESIS_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__GENESIS_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed genesis path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v genesis >/dev/null 2>&1",
    'printf "__GENESIS_PATH__=%s\\n" "$(command -v genesis)"',
    "genesis --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__GENESIS_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed genesis path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `genesis update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.genesis/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.GENESIS_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.GENESIS_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.GENESIS_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.GENESIS_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Genesis Control</title>") &&
          html.includes("<genesis-app></genesis-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Genesis Control</title>")} app=${html.includes("<genesis-app></genesis-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runGenesis(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "genesis.mjs");
}
