...
genesis_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${GENESIS_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "GENESIS_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
genesis_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
  ...
  if [ -d /app/dist-runtime/extensions ]; then
    export GENESIS_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
  elif [ -d /app/dist/extensions ]; then
    export GENESIS_BUNDLED_PLUGINS_DIR=/app/dist/extensions
  fi
}

genesis_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
genesis_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.genesis"
  ...
  export GENESIS_STATE_DIR="$dest_dir"
  export GENESIS_CONFIG_PATH="$dest_dir/genesis.json"
}
