...
function isSystemThrottleDisabled(env) {
  const normalized = env.GENESIS_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.GENESIS_VITEST_MAX_WORKERS ?? env.GENESIS_TEST_WORKERS);
  if (override !== null) {
