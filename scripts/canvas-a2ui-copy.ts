...
export function shouldSkipMissingA2uiAssets(env = process.env): boolean {
  return env.GENESIS_A2UI_SKIP_MISSING === "1" || Boolean(env.GENESIS_SPARSE_PROFILE);
}
...
export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because GENESIS_A2UI_SKIP_MISSING=1 or GENESIS_SPARSE_PROFILE is set.`,
      );
      return;
    }
