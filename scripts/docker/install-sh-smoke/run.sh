...
SMOKE_MODE="${GENESIS_INSTALL_SMOKE_MODE:-install}"
...
FRESH_VERSION="${GENESIS_INSTALL_FRESH_VERSION:-}"
FRESH_TAG_URL="${GENESIS_INSTALL_FRESH_TAG_URL:-}"
UPDATE_BASELINE_VERSION="${GENESIS_INSTALL_UPDATE_BASELINE:-latest}"
UPDATE_BASELINE_TAG_URL="${GENESIS_INSTALL_UPDATE_BASELINE_TAG_URL:-}"
UPDATE_EXPECT_VERSION="${GENESIS_INSTALL_UPDATE_EXPECT_VERSION:-}"
UPDATE_TAG_URL="${GENESIS_INSTALL_UPDATE_TAG_URL:-}"
HEARTBEAT_INTERVAL="${GENESIS_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"
INSTALL_COMMAND_TIMEOUT="${GENESIS_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"
  ...
  if [[ -n "$FRESH_VERSION" && -n "$FRESH_TAG_URL" ]]; then
    ...
    if [[ -n "${GENESIS_INSTALL_LATEST_OUT:-}" ]]; then
      ...
      if [[ -n "$latest_npm_version" ]]; then
        printf "%s" "$latest_npm_version" > "${GENESIS_INSTALL_LATEST_OUT:-}"
      else
        printf "%s" "$FRESH_VERSION" > "${GENESIS_INSTALL_LATEST_OUT:-}"
      fi
    fi
...
run_update_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: GENESIS_INSTALL_UPDATE_EXPECT_VERSION is required for update mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: GENESIS_INSTALL_UPDATE_TAG_URL is required for update mode" >&2
    return 1
  fi
  ...
  echo "==> Run genesis update from host-served tgz"
  local update_status
  ...
  set +e
  UPDATE_JSON="$(
    run_with_heartbeat "genesis update" \
      env npm_config_omit=optional NPM_CONFIG_OMIT=optional \
      genesis update --tag "$UPDATE_TAG_URL" --yes --json 2>"$update_stderr_file"
  )"
  update_status=$?
  set -e
  ...
  if [[ "$update_status" -ne 0 ]]; then
    echo "ERROR: genesis update failed with exit code $update_status" >&2
...
run_npm_global_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: GENESIS_INSTALL_UPDATE_EXPECT_VERSION is required for npm-global mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: GENESIS_INSTALL_UPDATE_TAG_URL is required for npm-global mode" >&2
    return 1
  fi
...
case "$SMOKE_MODE" in
  ...
  npm-global)
    ...
    ;;
  *)
    echo "ERROR: unsupported GENESIS_INSTALL_SMOKE_MODE=$SMOKE_MODE" >&2
    exit 1
    ;;
esac
