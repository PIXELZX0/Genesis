...
SKIP_PREVIOUS="${GENESIS_INSTALL_E2E_SKIP_PREVIOUS:-0}"
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
ANTHROPIC_API_TOKEN="${ANTHROPIC_API_TOKEN:-}"
AGENT_TURN_TIMEOUT_SECONDS="${GENESIS_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-600}"
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  GENESIS_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  GENESIS_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
...
dump_profile_debug() {
  ...
  echo "---- genesis processes ($profile) ----"
  for cmdline in /proc/[0-9]*/cmdline; do
    ...
    if [[ "$command" == *genesis* || "$command" == *node* ]]; then
      echo "$pid $command"
    fi
  done
}
...
run_profile() {
  ...
  SESSION_JSONL="$HOME/.genesis-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  for _ in $(seq 1 240); do
    if genesis --profile "$profile" health --timeout 5000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  genesis --profile "$profile" health --timeout 60000 --json >/dev/null
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "$HOME/.genesis-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
