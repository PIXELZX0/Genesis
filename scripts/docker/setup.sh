...
RAW_SANDBOX_SETTING="${GENESIS_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${GENESIS_DOCKER_SOCKET:-}"
TIMEZONE="${GENESIS_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$GENESIS_CONFIG_DIR/genesis.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == GENESIS_GATEWAY_TOKEN=* ]]; then
      token="${line#GENESIS_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${GENESIS_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$GENESIS_GATEWAY_PORT" "$GENESIS_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$GENESIS_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$GENESIS_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network genesis-cli service because it
  ...
  run_prestart_gateway --entrypoint node genesis-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" genesis-cli "$@"
}
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "GENESIS_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "GENESIS_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "GENESIS_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "GENESIS_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi
...
mkdir -p "$GENESIS_CONFIG_DIR/agents/main/agent"
mkdir -p "$GENESIS_CONFIG_DIR/agents/main/sessions"
...
export GENESIS_EXTENSIONS="${GENESIS_EXTENSIONS:-}"
export GENESIS_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export GENESIS_HOME_VOLUME="$HOME_VOLUME_NAME"
export GENESIS_ALLOW_INSECURE_PRIVATE_WS="${GENESIS_ALLOW_INSECURE_PRIVATE_WS:-}"
export GENESIS_SANDBOX="$SANDBOX_ENABLED"
export GENESIS_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export GENESIS_TZ="$TIMEZONE"
...
if [[ -z "${GENESIS_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    GENESIS_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $GENESIS_CONFIG_DIR/genesis.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      GENESIS_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${GENESIS_INSTALL_DOCKER_CLI:-}" ]]; then
    export GENESIS_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  GENESIS_EXTENSIONS \
  GENESIS_SANDBOX \
  GENESIS_DOCKER_SOCKET \
  DOCKER_GID \
  GENESIS_INSTALL_DOCKER_CLI \
  GENESIS_ALLOW_INSECURE_PRIVATE_WS \
  GENESIS_TZ

if [[ "$IMAGE_NAME" == "genesis:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "GENESIS_DOCKER_APT_PACKAGES=${GENESIS_DOCKER_APT_PACKAGES}" \
    --build-arg "GENESIS_EXTENSIONS=${GENESIS_EXTENSIONS}" \
    --build-arg "GENESIS_INSTALL_DOCKER_CLI=${GENESIS_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Genesis metadata subdirectory
# (.genesis/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh genesis-gateway -c \
  'find /home/node/.genesis -xdev -exec chown node:node {} +; \
   [ -d /home/node/.genesis/workspace/.genesis ] && chown -R node:node /home/node/.genesis/workspace/.genesis || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from GENESIS_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $GENESIS_GATEWAY_BIND"
...
# --- Sandbox setup (opt-in via GENESIS_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: genesis-sandbox:bookworm-slim"
    run_docker_build \
      -t "genesis-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker genesis-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg GENESIS_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (GENESIS_IMAGE=genesis:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: GENESIS_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Genesis config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate genesis-gateway
  fi
else
