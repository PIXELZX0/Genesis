...
// Usage:
// GENESIS_PROMPT_TRANSPORT=direct|gateway
// GENESIS_PROMPT_MODE=extra|override
// GENESIS_PROMPT_TEXT='...'
// GENESIS_PROMPT_CAPTURE=1
...
import { resolveGenesisAgentDir } from "../src/agents/agent-paths.js";
...
const TRANSPORT = process.env.GENESIS_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.GENESIS_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.GENESIS_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.GENESIS_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.GENESIS_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = process.env.GENESIS_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.GENESIS_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.GENESIS_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.GENESIS_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.GENESIS_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
...
if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing GENESIS_PROMPT_TEXT or GENESIS_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set GENESIS_LIVE_SETUP_TOKEN_VALUE or GENESIS_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["genesis.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        GENESIS_CONFIG_PATH: params.configPath,
        GENESIS_STATE_DIR: params.stateDir,
        GENESIS_AGENT_DIR: params.agentDir,
        GENESIS_GATEWAY_TOKEN: params.gatewayToken,
        ...
        GENESIS_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "genesis.json");
