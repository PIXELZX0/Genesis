...
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Genesis."
...
GUM_VERSION="${GENESIS_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Genesis Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://genesis.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$GENESIS_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.genesis.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_genesis_paths() {
    ...
    rm -rf "$npm_root"/.genesis-* "$npm_root"/genesis 2>/dev/null || true
}

extract_genesis_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_genesis_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/genesis" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/genesis"|"/usr/local/bin/genesis")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/genesis/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale genesis symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing genesis binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Genesis package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_genesis_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Genesis npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*genesis" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_genesis_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_genesis_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Genesis npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an genesis binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${GENESIS_TAGLINE_INDEX:-}" ]]; then
        if [[ "${GENESIS_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((GENESIS_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${GENESIS_NO_ONBOARD:-0}
NO_PROMPT=${GENESIS_NO_PROMPT:-0}
DRY_RUN=${GENESIS_DRY_RUN:-0}
INSTALL_METHOD=${GENESIS_INSTALL_METHOD:-}
GENESIS_VERSION=${GENESIS_VERSION:-latest}
USE_BETA=${GENESIS_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/genesis"
GIT_DIR=${GENESIS_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${GENESIS_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${GENESIS_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${GENESIS_VERBOSE:-0}"
VERIFY_INSTALL="${GENESIS_VERIFY_INSTALL:-0}"
GENESIS_BIN=""
PNPM_CMD=()
HELP=0
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/genesis)
...
Environment variables:
  ...
  GENESIS_NO_PROMPT=1
  GENESIS_VERIFY_INSTALL=1
  GENESIS_DRY_RUN=1
  GENESIS_NO_ONBOARD=1
  GENESIS_VERBOSE=1
  GENESIS_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Genesis checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Genesis source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_genesis_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"genesis"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://genesis.ai/install.sh | bash"
}
...
ensure_genesis_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/genesis" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/genesis" ]]; then
        ln -sf "$npm_root/genesis/dist/entry.js" "${npm_bin}/genesis"
        ui_info "Created genesis bin link at ${npm_bin}/genesis"
    fi
    return 0
}

# Check for existing Genesis installation
check_existing_genesis() {
    if [[ -n "$(type -P genesis 2>/dev/null || true)" ]]; then
        ui_info "Existing Genesis installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make genesis show as \"command not found\" in new terminals."
...
warn_genesis_not_found() {
    ui_warn "Installed, but genesis is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t genesis 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named genesis; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_genesis_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P genesis 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/genesis" ]]; then
        echo "${npm_bin}/genesis"
        return 0
    fi
...
install_genesis_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/PIXELZX0/Genesis.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Genesis from git checkout: ${repo_dir}"
    else
        ui_info "Installing Genesis from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Genesis" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Genesis" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/genesis" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/genesis"
    ui_success "Genesis wrapper installed to \$HOME/.local/bin/genesis"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:PIXELZX0/Genesis#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_genesis() {
    ...
    if can_resolve_registry_package_version "${GENESIS_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${GENESIS_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Genesis v${resolved_version}"
    else
        ui_info "Installing Genesis (${GENESIS_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${GENESIS_VERSION}")"

    if ! install_genesis_npm "${install_spec}"; then
    ...
    ensure_genesis_bin_link || true

    ui_success "Genesis installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${GENESIS_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_genesis_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (genesis not on PATH yet)"
        warn_genesis_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${GENESIS_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.genesis/workspace-${profile}"
    else
        echo "${HOME}/.genesis/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${GENESIS_CONFIG_PATH:-$HOME/.genesis/genesis.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run genesis onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but genesis not on PATH; skipping onboarding"
        warn_genesis_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run genesis onboard to retry"
        return
    }
}
...
resolve_genesis_version() {
    local version=""
    local raw_version_output=""
    local claw="${GENESIS_BIN:-}"
    if [[ -z "$claw" ]] && command -v genesis &> /dev/null; then
        claw="$(command -v genesis)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_genesis_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/genesis/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/genesis/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: genesis not on PATH yet"
        warn_genesis_not_found
        return 1
    fi

    run_quiet_step "Checking Genesis version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: genesis gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_genesis_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Genesis checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set GENESIS_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_genesis; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Genesis"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g genesis &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g genesis 2>/dev/null || true
        ...
        install_genesis_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/genesis" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/genesis"
    ...
    GENESIS_BIN="$(resolve_genesis_bin || true)"
    ...
    installed_version=$(resolve_genesis_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Genesis installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Genesis installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/genesis"
        ui_kv "Update command" "genesis update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running genesis doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                GENESIS_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                GENESIS_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                GENESIS_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run genesis doctor and genesis plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run genesis onboard later"
        else
            local config_path="${GENESIS_CONFIG_PATH:-$HOME/.genesis/genesis.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (genesis not on PATH yet)"
                    warn_genesis_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run genesis onboard to finish setup"
            return 0
        fi
    fi
    ...
    if command -v genesis &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (genesis daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if GENESIS_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: genesis daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${GENESIS_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
