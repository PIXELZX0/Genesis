...
const GENESIS_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncGenesisDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.genesis;
  if (!current || current === "workspace:*" || !GENESIS_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.genesis?.compat;
  const current = compat?.pluginApi;
  if (!current || !GENESIS_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildGenesisVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.genesis?.build;
  const current = build?.genesisVersion;
  if (!current) {
    return false;
  }
  ...
  build.genesisVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncGenesisDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncGenesisDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildGenesisVersionChanged = syncBuildGenesisVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildGenesisVersionChanged;
    if (!packageChanged) {
