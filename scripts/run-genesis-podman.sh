...
#   genesis --container genesis dashboard --no-open
#   genesis --container genesis channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      GENESIS_GATEWAY_TOKEN|GENESIS_PODMAN_CONTAINER|GENESIS_PODMAN_IMAGE|GENESIS_IMAGE|GENESIS_PODMAN_PULL|GENESIS_PODMAN_GATEWAY_HOST_PORT|GENESIS_GATEWAY_PORT|GENESIS_PODMAN_BRIDGE_HOST_PORT|GENESIS_BRIDGE_PORT|GENESIS_GATEWAY_BIND|GENESIS_PODMAN_USERNS|GENESIS_BIND_MOUNT_OPTIONS|GENESIS_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-genesis-podman.sh as your normal user so Podman stays rootless."
fi
...
PUBLISH_HOST="${GENESIS_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$GENESIS_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  GENESIS_CONTAINER="" GENESIS_CONFIG_DIR="$config_dir" \
    genesis config get gateway.bind 2>/dev/null || true
}
...
# GENESIS_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${GENESIS_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v genesis >/dev/null 2>&1; then
    echo "Warning: genesis not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    GENESIS_CONTAINER="" GENESIS_CONFIG_DIR="$config_dir" \
      genesis config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    GENESIS_CONTAINER="" GENESIS_CONFIG_DIR="$config_dir" \
      genesis config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  GENESIS_CONTAINER="" GENESIS_CONFIG_DIR="$config_dir" \
    genesis config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
SELINUX_MOUNT_OPTS=""
if [[ -z "${GENESIS_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${GENESIS_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$GENESIS_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.genesis/.npm \
    -e GENESIS_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.genesis:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.genesis/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$GENESIS_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME genesis dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME genesis devices approve --latest  # if pairing required"
