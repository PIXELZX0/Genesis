...
function resolveInstalledBinaryPath(prefixDir: string): string {
  return process.platform === "win32"
    ? join(prefixDir, "genesis.cmd")
    : join(prefixDir, "bin", "genesis");
}
...
function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".genesis", "genesis.json");
  mkdirSync(join(homeDir, ".genesis"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-genesis-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-genesis-release-check",
  });
  ...
  execFileSync(process.execPath, [join(packageRoot, "genesis.mjs"), "plugins", "doctor"], {
    cwd: packageRoot,
    stdio: "inherit",
    env,
  });
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "genesis-release-pack-smoke-"));
  try {
    ...
    execFileSync(
      process.execPath,
      [join(packageRoot, "genesis.mjs"), "completion", "--write-state"],
      {
        cwd: packageRoot,
        stdio: "inherit",
        env: {
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.genesis-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.genesis-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from genesis/plugin-sdk.
