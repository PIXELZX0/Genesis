...
type DriverMode = "token" | "webhook" | "genesis";
...
function resolveStateDir(): string {
  const override = process.env.GENESIS_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.GENESIS_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver genesis] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Genesis spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|genesis> Driver transport mode (default: token)\n" +
    ...
    "  --genesis-bin <path>        Genesis CLI binary for driver=genesis (default: genesis)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  GENESIS_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  GENESIS_DISCORD_SMOKE_DRIVER\n" +
    "  GENESIS_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  GENESIS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  GENESIS_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  GENESIS_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  GENESIS_DISCORD_SMOKE_AGENT\n" +
    "  GENESIS_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  GENESIS_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  GENESIS_DISCORD_SMOKE_POLL_MS\n" +
    "  GENESIS_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  GENESIS_DISCORD_SMOKE_GENESIS_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.GENESIS_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.GENESIS_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "genesis"
        ? "genesis"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.GENESIS_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.GENESIS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.GENESIS_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.GENESIS_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.GENESIS_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.GENESIS_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.GENESIS_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.GENESIS_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.GENESIS_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.GENESIS_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const genesisBin =
    resolveArg("--genesis-bin") || process.env.GENESIS_DISCORD_SMOKE_GENESIS_BIN || "genesis";
...
async function genesisCliJson<T>(params: { genesisBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.genesisBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`genesis ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithGenesis(params: {
  genesisBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await genesisCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    genesisBin: params.genesisBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "genesis") {
    return await readMessagesWithGenesis({
      genesisBin: params.args.genesisBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `genesis-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await genesisCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("genesis message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "genesis"
            ? await readMessagesWithGenesis({
                genesisBin: args.genesisBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Genesis.`,
        diagnostics: {
          bindingCandidates: [
            {
