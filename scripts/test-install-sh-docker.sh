...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${GENESIS_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$GENESIS_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.GENESIS_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `GENESIS_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${GENESIS_INSTALL_SMOKE_IMAGE:-genesis-install-smoke:local}"
...
PACKAGE_NAME="${GENESIS_INSTALL_PACKAGE:-genesis}"
SKIP_NONROOT="${GENESIS_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${GENESIS_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${GENESIS_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${GENESIS_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${GENESIS_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${GENESIS_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${GENESIS_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${GENESIS_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${GENESIS_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${GENESIS_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${GENESIS_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${GENESIS_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${GENESIS_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (GENESIS_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e GENESIS_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e GENESIS_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e GENESIS_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e GENESIS_INSTALL_SMOKE_MODE=update \
    -e GENESIS_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e GENESIS_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e GENESIS_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e GENESIS_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e GENESIS_NO_ONBOARD=1 \
    -e GENESIS_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (GENESIS_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e GENESIS_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e GENESIS_INSTALL_SMOKE_MODE=npm-global \
      -e GENESIS_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e GENESIS_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e GENESIS_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e GENESIS_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "${GENESIS_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
