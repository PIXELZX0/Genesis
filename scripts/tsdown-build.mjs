...
const logLevel = process.env.GENESIS_BUILD_VERBOSE ? "info" : "warn";
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveInteger(env.GENESIS_TSDOWN_TIMEOUT_MS);
  const heartbeatMs =
    parseNonNegativeInteger(env.GENESIS_TSDOWN_HEARTBEAT_MS) ?? DEFAULT_HEARTBEAT_MS;
  let timedOut = false;
  let settled = false;
