...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  GENESIS_SKIP_ACPX_RUNTIME: "1",
  GENESIS_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$GENESIS_WATCH_PID_FILE"',
    'mkdir -p "$GENESIS_HOME/.genesis"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$GENESIS_HOME/.genesis/genesis.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    GENESIS_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    GENESIS_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-gateway-watch-"));
