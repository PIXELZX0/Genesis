...
const skipPackValidationEnv = "GENESIS_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.bin?.genesis !== "genesis.mjs") {
    errors.push(
      `package.json bin.genesis must be "genesis.mjs"; found "${pkg.bin?.genesis ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`genesis-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `genesis-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
