...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "genesis.json");
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "genesis-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "genesis-bench",
    npm_config_update_notifier: "false",
    GENESIS_CONFIG: configPath,
    GENESIS_CONFIG_PATH: configPath,
    GENESIS_GATEWAY_STARTUP_TRACE: "1",
    GENESIS_HOME: root,
    GENESIS_LOCAL_CHECK: "0",
    GENESIS_STATE_DIR: path.join(root, "state"),
    GENESIS_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
  return env;
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "genesis-gateway-bench-"));
