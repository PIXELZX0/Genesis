...
IMAGE_NAME="$(docker_e2e_resolve_image "genesis-onboard-e2e" GENESIS_ONBOARD_E2E_IMAGE)"
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    set_isolated_genesis_env "$home_dir"
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/genesis-onboard-${label}.log"
  ...
  run_case_local_basic() {
    ...
    set_isolated_genesis_env "$home_dir"
    run_case_logged local-basic node "$GENESIS_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$GENESIS_STATE_DIR/workspace"
    config_path="$GENESIS_CONFIG_PATH"
    sessions_dir="$GENESIS_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_genesis_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$GENESIS_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$GENESIS_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
