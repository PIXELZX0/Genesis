...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/genesis-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/genesis-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/genesis-install.sh
  if [[ -n "$INSTALL_VERSION" ]]; then
    guest_exec /usr/bin/env GENESIS_NO_ONBOARD=1 bash /tmp/genesis-install.sh --version "$INSTALL_VERSION" --no-onboard
  else
    guest_exec /usr/bin/env GENESIS_NO_ONBOARD=1 bash /tmp/genesis-install.sh --no-onboard
  fi
  guest_exec genesis --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec genesis --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" genesis onboard \
    --non-interactive \
    --mode local \
...
set -euo pipefail
plugin_dir=/root/.genesis/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{
  "name": "@genesis/test-bad-plugin",
  "version": "1.0.0",
  "genesis": {
...
config_path = Path("/root/.genesis/genesis.json")
config = {}
...
verify_bad_plugin_diagnostic() {
  guest_bash_script <<'EOF'
grep -F "failed to load setup entry" /tmp/genesis-parallels-linux-gateway.log
EOF
}
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "genesis gateway run" >/dev/null 2>&1 || true
rm -f /tmp/genesis-parallels-linux-gateway.log
setsid sh -lc 'exec env GENESIS_HOME=/root GENESIS_STATE_DIR=/root/.genesis GENESIS_CONFIG_PATH=/root/.genesis/genesis.json ${API_KEY_ENV}=${api_key_value_q} genesis gateway run --bind loopback --port 18789 --force >/tmp/genesis-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec genesis gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec genesis gateway status --deep --require-rpc
    return
  fi
  guest_exec genesis gateway status --deep
}

verify_local_turn() {
  guest_exec genesis models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" genesis agent \
    --local \
    --agent main \
    --message ping \
    --json
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "genesis-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "genesis-main-upgrade.tgz"
