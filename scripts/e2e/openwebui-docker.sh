...
IMAGE_NAME="$(docker_e2e_resolve_image "genesis-openwebui-e2e" GENESIS_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${GENESIS_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${GENESIS_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${GENESIS_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${GENESIS_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${GENESIS_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${GENESIS_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="genesis-openwebui-e2e-$$"
GW_NAME="genesis-openwebui-gateway-$$"
OW_NAME="genesis-openwebui-$$"
DOCKER_COMMAND_TIMEOUT="${GENESIS_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-600s}"
DOCKER_PULL_TIMEOUT="${GENESIS_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
docker_cmd docker run -d \
  ...
  -e "GENESIS_OPENWEBUI_MODEL=$MODEL" \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/genesis-openwebui-config.XXXXXX.json)"
    GENESIS_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.GENESIS_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.GENESIS_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.GENESIS_OPENWEBUI_MODEL },
];
...
EOF
    cat > "$workspace/.genesis/workspace-state.json" <<'"'"'EOF'"'"'
{
  "version": 1,
...
docker_cmd docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Genesis E2E" \
  -e WEBUI_SECRET_KEY="genesis-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Genesis E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="genesis/default" \
...
echo "Running Open WebUI -> Genesis smoke..."
