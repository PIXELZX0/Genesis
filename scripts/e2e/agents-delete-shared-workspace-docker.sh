...
IMAGE_NAME="$(docker_e2e_resolve_image "genesis-agents-delete-shared-workspace-e2e:local" GENESIS_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE)"
SKIP_BUILD="${GENESIS_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD:-0}"
DOCKER_COMMAND_TIMEOUT="${GENESIS_AGENTS_DELETE_SHARED_WORKSPACE_DOCKER_COMMAND_TIMEOUT:-300s}"
...
run_logged agents-delete-shared-workspace docker_cmd docker run --rm \
  ...
  -e GENESIS_SKIP_PROVIDERS=1 \
  -e GENESIS_SKIP_GMAIL_WATCHER=1 \
  -e GENESIS_SKIP_CRON=1 \
  -e GENESIS_SKIP_CANVAS_HOST=1 \
  -e GENESIS_SKIP_BROWSER_CONTROL_SERVER=1 \
  -e GENESIS_SKIP_ACPX_RUNTIME=1 \
  -e GENESIS_SKIP_ACPX_RUNTIME_PROBE=1 \
  "$IMAGE_NAME" \
  -lc '
set -euo pipefail
...
run_genesis() {
  ...
  if [ -f /app/genesis.mjs ]; then
    node /app/genesis.mjs "$@"
    return
  fi
  echo "genesis CLI not found in Docker image" >&2
  exit 1
}

home_dir="$(mktemp -d /tmp/genesis-agents-delete-e2e-home.XXXXXX)"
export HOME="$home_dir"
export GENESIS_HOME="$home_dir"
export GENESIS_STATE_DIR="$home_dir/.genesis"
export SHARED_WORKSPACE="$home_dir/workspace-shared"
output_file="$home_dir/delete.json"
trap '\''rm -rf "$home_dir"'\'' EXIT

mkdir -p "$GENESIS_STATE_DIR" "$SHARED_WORKSPACE"
node --input-type=module - <<'\''NODE'\''
import fs from "node:fs";
import path from "node:path";

const stateDir = process.env.GENESIS_STATE_DIR;
const sharedWorkspace = process.env.SHARED_WORKSPACE;
if (!stateDir || !sharedWorkspace) {
  throw new Error("missing GENESIS_STATE_DIR or SHARED_WORKSPACE");
}
...
run_genesis agents delete ops --force --json > "$output_file"
...
const configPath = path.join(process.env.GENESIS_STATE_DIR, "genesis.json");
