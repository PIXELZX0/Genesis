...
import { applyDockerOpenAiProviderConfig, type GenesisConfig } from "./docker-openai-seed.ts";
...
process.title = "genesis-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.GENESIS_STATE_DIR?.trim() || path.join(os.homedir(), ".genesis");
  const configPath = process.env.GENESIS_CONFIG_PATH?.trim() || path.join(stateDir, "genesis.json");
