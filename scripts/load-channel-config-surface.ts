...
function copyModuleImportGraphWithoutNodeModules(params: {
  ...
  const tempParent = path.join(params.repoRoot, ".genesis-config-doc-cache");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.GENESIS_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing GENESIS_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        GENESIS_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
