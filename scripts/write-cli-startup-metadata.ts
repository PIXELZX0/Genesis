...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.genesis?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.genesis?.channel?.order;
      const labelRaw = parsed.genesis?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".genesis-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "genesis-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "genesis-build",
    ...
    GENESIS_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    GENESIS_PLUGIN_DISCOVERY_CACHE_MS: "0",
    GENESIS_PLUGIN_MANIFEST_CACHE_MS: "0",
