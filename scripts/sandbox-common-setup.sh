...
GENESIS_DOCKER_BUILD_USE_BUILDX="${GENESIS_DOCKER_BUILD_USE_BUILDX:-0}"
GENESIS_DOCKER_BUILD_CACHE_FROM="${GENESIS_DOCKER_BUILD_CACHE_FROM:-}"
GENESIS_DOCKER_BUILD_CACHE_TO="${GENESIS_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${GENESIS_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${GENESIS_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${GENESIS_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${GENESIS_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${GENESIS_DOCKER_BUILD_CACHE_TO}")
  fi
fi
