...
function createTempHome(): string {
  return makeTempDir(tempDirs, "genesis-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-genesis.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.genesis/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.genesis/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".genesis", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".genesis", "external-plugins", "glueclaw", "genesis.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".genesis", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.GENESIS_LIVE_TEST_QUIET = "1";
    process.env.GENESIS_CONFIG_PATH = "~/custom-genesis.json5";
    process.env.GENESIS_TEST_HOME = priorIsolatedHome;
    process.env.GENESIS_STATE_DIR = path.join(priorIsolatedHome, ".genesis");
    ...
    expect(process.env.GENESIS_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".genesis", "genesis.json");
    ...
    expect(fs.existsSync(path.join(testEnv.tempHome, ".genesis", "credentials", "token.txt"))).toBe(
      true,
    );
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".genesis", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.GENESIS_LIVE_TEST = "1";
    process.env.GENESIS_LIVE_USE_REAL_HOME = "1";
    process.env.GENESIS_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.GENESIS_LIVE_TEST;
    delete process.env.GENESIS_LIVE_GATEWAY;
    delete process.env.GENESIS_LIVE_USE_REAL_HOME;
    delete process.env.GENESIS_LIVE_TEST_QUIET;
