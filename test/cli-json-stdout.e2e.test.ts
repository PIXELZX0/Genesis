  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.GENESIS_HOME;
        delete env.GENESIS_STATE_DIR;
        delete env.GENESIS_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "genesis.mjs");
