...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("genesis-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@genesis/whatsapp",
      version: "2026.3.23",
      description: "Genesis WhatsApp channel plugin",
      genesis: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(buildOfficialChannelCatalog({ repoRoot }).entries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          genesis: expect.objectContaining({
            channel: expect.objectContaining({
              id: "wecom",
              label: "WeCom",
            }),
            install: {
              npmSpec: "@wecom/wecom-genesis-plugin@2026.4.23",
              defaultChoice: "npm",
              expectedIntegrity:
  ...
  it("keeps official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("genesis-official-channel-catalog-policy-");
    ...
    for (const entry of entries) {
      const installSource = describePluginInstallSource(entry.genesis?.install ?? {});
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("genesis-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@genesis/whatsapp",
      genesis: {
        channel: {
