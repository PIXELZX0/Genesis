...
export function openAiPluginPersonalityConfig(personality: "friendly" | "off"): GenesisConfig {
  return {
    plugins: {
      entries: {
        openai: {
...
export function sharedGpt5PersonalityConfig(personality: "friendly" | "off"): GenesisConfig {
  return {
    agents: {
      defaults: {
        promptOverlays: {
...
export function codexPromptOverlayContext(params?: {
  ...
  return {
    ...
    promptMode: "full",
    agentDir: "/tmp/genesis-codex-prompt-contract-agent",
    workspaceDir: "/tmp/genesis-codex-prompt-contract-workspace",
