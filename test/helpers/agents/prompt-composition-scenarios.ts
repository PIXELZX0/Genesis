...
async function createToolRichScenario(workspaceDir: string): Promise<PromptScenario> {
  const skillsPrompt = [
    ...
    "<skill><name>release</name><description>Release Genesis safely.</description><location>/skills/release/SKILL.md</location></skill>",
    "</available_skills>",
  ].join("\n");
...
async function createMaintenanceScenario(workspaceDir: string): Promise<PromptScenario> {
  ...
  const memoryFlushPrompt = [
    ...
    "Current time: Sunday, March 15th, 2026 - 9:30 PM (UTC-7) / 2026-03-16 04:30 UTC",
  ].join("\n");
...
export async function createWorkspaceWithPromptCompositionFiles(): Promise<string> {
  const workspaceDir = await makeTempWorkspace("genesis-prompt-cache-");
  await writeWorkspaceFile({
    dir: workspaceDir,
    name: "AGENTS.md",
    content: [
...
export async function createPromptCompositionScenarios(): Promise<{
  ...
  const warningWorkspaceDir = await makeTempWorkspace("genesis-prompt-cache-warning-");
  const scenarios = [
