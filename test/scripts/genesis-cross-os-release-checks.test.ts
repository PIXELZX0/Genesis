...
} from "../../scripts/genesis-cross-os-release-checks.ts";

describe("scripts/genesis-cross-os-release-checks", () => {
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "genesis-cross-os-agent-output-"));
    try {
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'genesis.cmd')");
    expect(script).toContain("$cmd = Get-Command genesis -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        GENESIS_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        GENESIS_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        GENESIS_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("genesis-2026.4.14.tgz")).toBe("application/octet-stream");
  });
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/genesis", "darwin"),
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "genesis-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "genesis-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
