...
function makeTempRepo() {
  const dir = mkdtempSync(path.join(tmpdir(), "genesis-root-deps-audit-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("collectRootDependencyOwnershipCheckErrors", () => {
  it("catches dependencies mirrored at root but only imported by one extension", () => {
    ...
    writeRepoFile(
      repoRoot,
      "extensions/regional-chat/package.json",
      JSON.stringify({ dependencies: { "vendor-sdk": "^1.0.0" } }),
    );
    writeRepoFile(
      repoRoot,
      "extensions/regional-chat/src/setup.ts",
    ...
    expect(collectRootDependencyOwnershipCheckErrors(records)).toEqual([
      "root dependency 'vendor-sdk' is extension-owned (remove from root package.json and rely on owning extension manifests plus doctor --fix); extension declarations: regional-chat:dependencies; sample imports: extensions/regional-chat/src/setup.ts",
    ]);
  });
  ...
  it("fails only extension-owned root dependencies", () => {
    expect(
      collectRootDependencyOwnershipCheckErrors([
        {
          category: "extension_only_localizable",
          declaredInExtensions: ["regional-chat:dependencies"],
          depName: "vendor-sdk",
          recommendation:
            "remove from root package.json and rely on owning extension manifests plus doctor --fix",
          sampleFiles: ["extensions/regional-chat/src/bridge/setup/finalize.ts"],
        },
        {
          category: "unreferenced",
      ...
      "root dependency 'vendor-sdk' is extension-owned (remove from root package.json and rely on owning extension manifests plus doctor --fix); extension declarations: regional-chat:dependencies; sample imports: extensions/regional-chat/src/bridge/setup/finalize.ts",
    ]);
  });
});
