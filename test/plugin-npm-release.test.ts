...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(parsePluginReleaseSelection(" @genesis/zalo, @genesis/feishu  @genesis/zalo ")).toEqual([
      "@genesis/feishu",
      "@genesis/zalo",
    ]);
  });
...
describe("parsePluginReleaseArgs", () => {
  ...
  it("rejects plugin names when all-publishable mode is selected", () => {
    expect(() =>
      parsePluginReleaseArgs(["--selection-mode", "all-publishable", "--plugins", "@genesis/zalo"]),
...
describe("collectPublishablePluginPackages", () => {
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "genesis-plugin-npm-release-");
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      name: "@genesis/demo-plugin",
      version: "2026.4.10",
      genesis: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@genesis/demo-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@genesis/demo-plugin",
        version: "2026.4.10",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@genesis/demo-plugin",
      },
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@genesis/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @genesis/missing.");
  });
});
