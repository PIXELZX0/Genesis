...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "genesis-postpublish-context-engine-"));
  }
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("Genesis 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("Genesis 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/genesis-prefix", "darwin")).toBe(
      "/tmp/genesis-prefix/bin/genesis",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/genesis-prefix", "win32")).toBe(
      "C:/genesis-prefix/genesis.cmd",
    );
  });
});
...
describe("collectInstalledMirroredRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "genesis-postpublish-installed-"));
  }
  ...
  it("skips manifest-only sidecar directories without package.json", () => {
    ...
    try {
      ...
      writePackageFile(packageRoot, "dist/extensions/device-pair/genesis.plugin.json", {
        id: "device-pair",
      });
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "genesis-postpublish-root-deps-"));
  }
