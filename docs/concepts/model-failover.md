...
For a normal text run, Genesis evaluates candidates in this order:
...
Inside each candidate, Genesis tries auth-profile failover before advancing to
...
- Runtime auth-routing state lives in `~/.genesis/agents/<agentId>/agent/auth-state.json`.
...
returning control to Genesis. For Stainless-based SDKs such as Anthropic and
OpenAI, Genesis caps SDK-internal `retry-after-ms` / `retry-after` waits at 60
...
`GENESIS_SDK_RETRY_MAX_WAIT_SECONDS`; see [/concepts/retry](/concepts/retry).

Rate-limit cooldowns can also be model-scoped:

- Genesis records `cooldownModel` for rate-limit failures when the failing
  model id is known.
...
here. Genesis keeps explicit billing text in the billing lane even when a
...
cooldowns. By default, Genesis allows one same-provider auth-profile retry,
...
Genesis builds the candidate list from the currently requested `provider/model`
...
- If the current run is already on a configured fallback in the same provider
  family, Genesis keeps using the full configured chain.
- If the current run is on a different provider than config and that current
  model is not already part of the configured fallback chain, Genesis does not
...
When every auth profile for a provider is already in cooldown, Genesis does
...
When every candidate fails, Genesis throws `FallbackSummaryError`. The outer
...
- if the remaining block is a matching model-scoped rate limit, Genesis
