---
summary: "Delegate architecture: running Genesis as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Genesis as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Genesis agent that:
...
Genesis's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Genesis's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.genesis/workspace-delegate",
  tools: {
...
- Cron run history: `~/.genesis/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.genesis/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Genesis's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
genesis agents add delegate
```

This creates:

- Workspace: `~/.genesis/workspace-delegate`
- State: `~/.genesis/agents/delegate/agent`
- Sessions: `~/.genesis/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.genesis/workspace" },
      {
        id: "delegate",
        workspace: "~/.genesis/workspace-delegate",
        tools: {
...
~/.genesis/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.genesis/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.genesis/workspace-org",
        agentDir: "~/.genesis/agents/org-assistant/agent",
...
recall view. Genesis redacts credential/token-like text, truncates long
