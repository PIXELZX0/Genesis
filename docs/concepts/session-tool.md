...
Genesis gives agents tools to work across sessions, inspect status, and
...
After the target responds, Genesis can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned Genesis
...
the completion origin only identifies a channel Genesis can still reuse the
