...
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai-codex:default` profile, but once Genesis has a local OAuth profile,
...
- Legacy compatibility file: `~/.genesis/agents/<agentId>/agent/auth.json`
...
All of the above also respect `$GENESIS_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)
...
Claude subscription limits, and Anthropic staff told us Genesis-style Claude
CLI usage is allowed again. Genesis therefore treats Claude CLI reuse and
...
If you want other subscription-style options in Genesis, see [OpenAI
...
Genesis supports Anthropic Claude CLI reuse again. If you already have a local
...
- exception: some external CLI credentials stay externally managed; Genesis
  ...
  `openai-codex:default` profile, then Genesis-owned refreshes keep the local
  profile canonical.
