---
summary: "How Genesis remembers things across sessions"
...
Genesis remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.genesis/workspace`).
...
<Info>
Genesis auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Genesis
...
```bash
genesis memory rem-backfill --rollback
genesis memory rem-backfill --rollback-short-term
```

## CLI

```bash
genesis memory status          # Check index status and provider
genesis memory search "query"  # Search from the command line
genesis memory index --force   # Rebuild the index
```
