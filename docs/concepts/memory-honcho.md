...
[Honcho](https://honcho.dev) adds AI-native memory to Genesis. It persists
...
```bash
genesis plugins install @honcho-ai/genesis-honcho
genesis honcho setup
...
Settings live under `plugins.entries["genesis-honcho"].config`:
    ...
    entries: {
      "genesis-honcho": {
        config: {
          ...
          workspaceId: "genesis", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `genesis honcho setup` detects and
...
```bash
genesis honcho setup                        # Configure API key and migrate files
genesis honcho status                       # Check connection status
genesis honcho ask <question>               # Query Honcho about the user
genesis honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/genesis-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Genesis integration guide](https://docs.honcho.dev/v3/guides/integrations/genesis)
- [Memory](/concepts/memory) -- Genesis memory overview
