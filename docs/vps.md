---
summary: "Run Genesis on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Genesis
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Genesis Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/genesis-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/genesis-compile-cache
mkdir -p /var/tmp/genesis-compile-cache
export GENESIS_NO_RESPAWN=1
EOF
source ~/.bashrc
```

- `NODE_COMPILE_CACHE` improves repeated command startup times.
- `GENESIS_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `GENESIS_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/genesis-compile-cache`
...
For the standard `genesis onboard --install-daemon` path, edit the user unit:

```bash
systemctl --user edit genesis-gateway.service
```

```ini
[Service]
Environment=GENESIS_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/genesis-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
`genesis-gateway.service` via `sudo systemctl edit genesis-gateway.service`.
