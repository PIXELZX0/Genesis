...
Genesis supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Genesis also supports reusing a local Claude CLI login.
...
Anthropic setup-token auth is still available in Genesis as a supported token
path. Anthropic staff has since told us that Genesis-style Claude CLI usage is
allowed again, so Genesis treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Genesis to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Genesis auth profile.
...
- If auth exists but Genesis cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Genesis integration path is allowed again.
- Genesis therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
When you debug order issues, `genesis models status --probe` shows omitted
...
Run `genesis models status` to confirm which profile is expiring. If an
