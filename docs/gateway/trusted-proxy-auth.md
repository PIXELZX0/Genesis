...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Genesis can verify.
...
- Genesis can stay on loopback HTTP behind the proxy.
...
If Genesis itself serves HTTPS directly (no TLS-terminating proxy), set:
...
Genesis rejects ambiguous configurations where both a `gateway.auth.token` (or `GENESIS_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-genesis-scopes`.

Examples:

- `x-genesis-scopes: operator.read`
- `x-genesis-scopes: operator.read,operator.write`
- `x-genesis-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Genesis honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-genesis-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-genesis-scopes` explicitly when you want a trusted-proxy request to
...
Genesis rejected a loopback-source trusted-proxy request.
