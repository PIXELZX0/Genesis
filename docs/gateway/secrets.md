...
Genesis supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `GENESIS_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
- Wallet network credentials such as RPC URLs, API keys, and
  `monero-wallet-rpc` auth fields follow the same active-surface filtering when
  `wallet.enabled` and the matching `wallet.networks.<chain>.enabled` setting
  make the chain active. The local wallet mnemonic/private key keystore is
  separate from SecretRefs and is encrypted under the Genesis credentials
  directory.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Genesis runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Genesis validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Genesis QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/genesis"],
...
- Genesis resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__GENESIS_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `genesis.json` refs)
...
When reload-time activation fails after a healthy state, Genesis enters degraded secrets state.
...
- Strict command paths (for example `genesis memory` remote-memory paths and `genesis qr --remote` when it needs remote shared-secret refs) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `genesis status`, `genesis status --all`, `genesis channels status`, `genesis channels resolve`, `genesis security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `genesis secrets reload`.
...
```bash
genesis secrets audit --check
genesis secrets configure
genesis secrets audit --check
```
...
- plaintext values at rest (`genesis.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `genesis.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `genesis secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `genesis.json` plus `auth-profiles.json` for one agent scope
...
- `genesis secrets configure --providers-only`
- `genesis secrets configure --skip-provider-setup`
- `genesis secrets configure --agent <id>`
...
```bash
genesis secrets apply --from /tmp/genesis-secrets-plan.json
genesis secrets apply --from /tmp/genesis-secrets-plan.json --allow-exec
genesis secrets apply --from /tmp/genesis-secrets-plan.json --dry-run
genesis secrets apply --from /tmp/genesis-secrets-plan.json --dry-run --allow-exec
```
...
Genesis intentionally does not write rollback backups containing historical plaintext secret values.
