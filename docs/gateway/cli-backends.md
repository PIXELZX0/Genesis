...
- **Genesis tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
genesis agent --message "hi" --model codex-cli/gpt-5.5
```
...
gateway host, Genesis now auto-loads the owning bundled plugin when your config
...
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   ...
   Genesis session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us Genesis-style Claude CLI usage is allowed again, so Genesis treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes Genesis's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so Genesis writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the Genesis skills snapshot
two ways: the compact Genesis skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Genesis would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Genesis to the
child process environment for the run.

Claude CLI also has its own noninteractive permission mode. Genesis maps that
...
`tools.exec.ask: "off"`), Genesis adds `--permission-mode bypassPermissions`.
...
Before Genesis can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
genesis models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, Genesis resumes from the stored Claude session id. Stored session
...
- Genesis drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, Genesis lets that CLI enforce resume permissions.
...
- For Gemini CLI JSON output, Genesis reads reply text from `response` and
...
rewrites streamed assistant deltas and parsed final text before Genesis handles
...
CLI backends do **not** receive Genesis tool calls directly, but a backend can
...
If no MCP servers are enabled, Genesis still injects a strict config when a
...
- **No direct Genesis tool calls.** Genesis does not inject tool calls into
