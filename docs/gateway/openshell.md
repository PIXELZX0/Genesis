---
summary: "Use OpenShell as a managed sandbox backend for Genesis agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Genesis. Instead of running Docker
containers locally, Genesis delegates sandbox lifecycle to the `openshell` CLI,
...
- Genesis Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Genesis creates an OpenShell
...
- You edit files locally outside Genesis and want those changes visible in the
...
- When the sandbox is first created, Genesis seeds the remote workspace from
...
- Genesis does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Genesis after the initial seed,
the remote sandbox does **not** see those changes. Use
`genesis sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"genesis"`   | Sandbox source for first-time create                  |
...
1. Genesis calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Genesis calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `genesis sandbox` commands
