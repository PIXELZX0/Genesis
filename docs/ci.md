...
environment, and the Telegram lane uses Convex leases. `Genesis Release
...
When version-only release metadata files appear alongside code changes,
`check:changed` first validates those release metadata files with the release
guards, then routes the remaining code paths normally so a package/app version
bump does not promote an otherwise scoped diff to the full Vitest suite.
