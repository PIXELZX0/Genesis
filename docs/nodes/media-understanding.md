...
Vendor-specific media behavior is registered by vendor plugins, while Genesis
...
- If the active primary image model already supports vision natively, Genesis
...
- Explicit `genesis infer image describe --model <provider/model>` requests
...
5. **Provider auth**
   ...
   - Ollama image understanding is available when selected explicitly, for
     ...
     `genesis infer image describe --model ollama/<vision-model>`.
...
When provider-based **audio** and **video** media understanding is enabled, Genesis
...
If the proxy value is malformed, Genesis logs a warning and falls back to direct
fetch.
...
- `parakeet-mlx` note: with `--output-dir`, Genesis reads `<output-dir>/<media-basename>.txt` when output format is `txt` (or unspecified); non-`txt` formats fall back to stdout.
...
- If a file has no extractable text, Genesis injects `[No extractable text]`.
