...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`genesis approvals get --node ...`), not in the gateway pairing record.
