...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Genesis
...
Today this applies to PI embedded runs and Genesis dynamic tools in the Codex
app-server harness. Tokenjuice hooks Genesis's tool-result middleware and
...
Genesis already ships the plugin. There is no separate `plugins install`
or `tokenjuice install genesis` step.
