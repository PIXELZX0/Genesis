...
If your Ollama host expects bearer auth, Genesis reuses
...
- If the Ollama host is auth-protected, Genesis reuses the normal Ollama
  provider API key when present.
- Genesis warns during setup if Ollama is unreachable or not signed in, but
