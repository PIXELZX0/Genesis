...
If you explicitly opt `jq` into `safeBins`, Genesis still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, Genesis sends a followup `agent` turn to the same session.
...
topics, Genesis preserves the topic for the approval prompt and the post-approval follow-up.
