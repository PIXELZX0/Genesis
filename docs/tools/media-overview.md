...
Genesis generates images, videos, and music, understands inbound media (images, audio, video), and speaks replies aloud with text-to-speech. All media capabilities are tool-driven: the agent decides when to use them based on the conversation, and each tool only appears when at least one backing provider is configured.
...
Video and music generation run as background tasks because provider processing typically takes 30 seconds to several minutes. When the agent calls `video_generate` or `music_generate`, Genesis submits the request to the provider, returns a task ID immediately, and tracks the job in the task ledger. The agent continues responding to other messages while the job runs. When the provider finishes, Genesis wakes the agent so it can post the finished media back into the original channel. Image generation and TTS are synchronous and complete inline with the reply.
...
Google maps to Genesis's image, video, music, batch TTS, backend realtime
voice, and media-understanding surfaces. OpenAI maps to Genesis's image,
video, batch TTS, batch STT, Voice Call streaming STT, backend realtime voice,
and memory embedding surfaces. xAI currently maps to Genesis's image, video,
...
registered in Genesis until the shared realtime voice contract can represent
it.
