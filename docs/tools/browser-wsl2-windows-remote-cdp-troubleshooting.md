...
read_when:
  - Running Genesis Gateway in WSL2 while Chrome lives on Windows
...
- Genesis Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/PIXELZX0/Genesis/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Genesis and Chrome are on the same machine
...
- Genesis points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Genesis is not the problem yet.
...
Fix that before touching Genesis config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Genesis at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Genesis to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Genesis to succeed
...
```bash
genesis browser open https://example.com --browser-profile remote
genesis browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `genesis browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `genesis browser stop --browser-profile remote`
...
3. Genesis config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Genesis config or Control UI auth
