...
- On completion, Genesis best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.
...
- Completion delivery keeps the resolved requester route:
  ...
  - if the completion origin only provides a channel, Genesis fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, Genesis uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, Genesis falls back to that session's requester when available
...
Genesis does not treat `endedAt` absence as permanent proof that a sub-agent
