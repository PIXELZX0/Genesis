...
Genesis supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Genesis also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `genesis configure --section web`. It stores the key in
`~/.genesis/genesis.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.genesis/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
