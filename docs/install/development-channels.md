...
- **`dev`**: ensures a git checkout (default `~/genesis`, override with
  `GENESIS_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
genesis update --tag 2026.4.1-beta.1
...
- The tag is not persisted. Your next `genesis update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Genesis prompts for confirmation (skip with `--yes`).
...
Preview what `genesis update` would do without making changes:

```bash
genesis update --dry-run
genesis update --channel beta --dry-run
genesis update --tag 2026.4.1-beta.1 --dry-run
genesis update --dry-run --json
```
...
When you switch channels with `genesis update`, Genesis also syncs plugin
sources:
