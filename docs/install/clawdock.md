---
summary: "ClawDock shell helpers for Docker-based Genesis installs"
read_when:
  - You run Genesis with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based Genesis installs.
...
| `clawdock-cli <command>`  | Run Genesis CLI commands in Docker            |
...
- `~/.genesis/.env` for env-backed provider keys and bot tokens
- `~/.genesis/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.genesis/genesis.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `genesis.json` quickly. It redacts `.env` values in its printed output.
