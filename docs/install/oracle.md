---
summary: "Host Genesis on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Genesis on Oracle Cloud
  - Looking for free VPS hosting for Genesis
  - Want 24/7 Genesis on a small server
title: "Oracle Cloud"
---

Run a persistent Genesis Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@genesis`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=genesis --reset` to re-authenticate.

**Gateway will not start** -- Run `genesis doctor --non-interactive` and check logs with `journalctl --user -u genesis-gateway.service -n 50`.
