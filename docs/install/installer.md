...
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + Genesis into a local prefix (`~/.genesis`) with npm or git checkout modes. No root required. |
...
<Tabs>
  ...
  <Tab title="Git install">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://genesis.ai/install-cli.sh | bash -s -- --install-method git --git-dir ~/genesis
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab title="GitHub main via npm">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://genesis.ai/install.ps1))) -Tag main
    ```
  </Tab>
