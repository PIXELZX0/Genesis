---
summary: "Deploy Genesis Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Genesis on a Kubernetes cluster
  - You want to test Genesis in a Kubernetes environment
title: "Kubernetes"
---

# Genesis on Kubernetes

A minimal starting point for running Genesis on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Genesis is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/genesis 18789:18789 -n genesis
...
```bash
kubectl get secret genesis-secrets -n genesis -o jsonpath='{.data.GENESIS_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: genesis (configurable via GENESIS_NAMESPACE)
├── Deployment/genesis        # Single pod, init container + gateway
├── Service/genesis           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/genesis-config  # genesis.json + AGENTS.md
└── Secret/genesis-secrets    # Gateway token + API keys
```
...
Edit `genesis.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret genesis-secrets -n genesis \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/genesis -n genesis
```

### Custom namespace

```bash
GENESIS_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/genesis/genesis:latest # or pin to a specific version from https://github.com/PIXELZX0/Genesis/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # genesis.json + AGENTS.md
