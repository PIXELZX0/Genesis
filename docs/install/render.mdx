---
summary: "Deploy Genesis on Render with Infrastructure-as-Code"
read_when:
  - Deploying Genesis to Render
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/PIXELZX0/Genesis)
...
your fork's `render.yaml` (but note: no persistent disk means Genesis state
...
`GENESIS_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
- Missing `GENESIS_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `GENESIS_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `genesis backup create` in the Render shell.
