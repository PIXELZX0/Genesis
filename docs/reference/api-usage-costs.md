...
- Gemini CLI note: when the CLI returns JSON output, Genesis reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us Genesis-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that Genesis can
show in `/usage full`.
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Genesis inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Genesis prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Genesis falls back to matching OAuth/API-key
...
Genesis's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Genesis Claude-login path with **Extra Usage** enabled.
