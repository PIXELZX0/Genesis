...
This is the full reference for `genesis onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.genesis/genesis.json` exists, choose **Keep / Modify / Reset**.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Genesis now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.genesis/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.genesis/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.genesis/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$GENESIS_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
  ...
  <Step title="Web search">
    ...
    - Configure later: `genesis configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    ...
    - Tip: `genesis status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  </Step>
...
```bash
export GENESIS_GATEWAY_TOKEN="your-token"
genesis onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env GENESIS_GATEWAY_TOKEN
```
...
Typical fields in `~/.genesis/genesis.json`:
