---
summary: "CLI reference for `genesis health` (gateway health snapshot via RPC)"
read_when:
...
genesis health
genesis health --json
genesis health --timeout 2500
genesis health --verbose
genesis health --debug
```

Notes:

- Default `genesis health` asks the running gateway for its health snapshot. When the
