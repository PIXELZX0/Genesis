...
genesis doctor --deep
genesis doctor --repair --non-interactive
...
- Doctor includes a memory-search readiness check and can recommend `genesis configure --section model` when embedding credentials are missing.
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `genesis config set agents.defaults.sandbox.mode off`).
