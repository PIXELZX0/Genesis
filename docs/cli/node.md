...
`genesis node run` and `genesis node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `GENESIS_GATEWAY_TOKEN` / `GENESIS_GATEWAY_PASSWORD` are checked first.
...
For approved async node exec, Genesis prepares a canonical `systemRunPlan`
