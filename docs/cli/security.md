...
genesis security audit --deep --password <password>
genesis security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Genesis is a personal-assistant trust model by default.
...
- tightens permissions for state/config and common sensitive files
  ...
  `*.jsonl`)
- also tightens config include files referenced from `genesis.json`
