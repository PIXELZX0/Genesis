...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Genesis Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`genesis acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Genesis channel
conversations instead of hosting an ACP harness session, use
[`genesis mcp serve`](/cli/mcp) instead.
...
- Genesis acts as an ACP server
- an IDE or ACP client connects to Genesis
- Genesis forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Genesis runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Genesis: use `genesis acp`
- Genesis should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Genesis gateway or agent instead.                                                                                                                                      |
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Genesis through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Genesis Gateway session.
...
If you want ACPX-backed sessions to see Genesis plugin tools or selected
...
[Genesis tools MCP bridge](/tools/acp-agents-setup#genesis-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Genesis bot over ACP, use `acpx` with its built-in `genesis` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx genesis` at `genesis acp`.
3. Target the Genesis session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Genesis ACP session
acpx genesis exec "Summarize the active Genesis session state."

# Persistent named session for follow-up turns
acpx genesis sessions ensure --name codex-bridge
acpx genesis -s codex-bridge --cwd /path/to/repo \
  "Ask my Genesis work agent for recent context relevant to this repo."
```

If you want `acpx genesis` to target a specific Gateway and session key every
time, override the `genesis` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "genesis": {
      "command": "env GENESIS_HIDE_BANNER=1 GENESIS_SUPPRESS_NOTES=1 genesis acp --url ws://127.0.0.1:18789 --token-file ~/.genesis/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Genesis checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env GENESIS_HIDE_BANNER=1 GENESIS_SUPPRESS_NOTES=1 node genesis.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Genesis agent without scraping a terminal.
...
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`GENESIS_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `GENESIS_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `genesis acp client` sets `GENESIS_SHELL=acp-client` on the spawned bridge process.
