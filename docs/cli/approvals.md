---
summary: "CLI reference for `genesis approvals` and `genesis exec-policy`"
read_when:
...
`genesis exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.genesis/exec-approvals.json`

Examples:

```bash
genesis exec-policy show
genesis exec-policy show --json

genesis exec-policy preset yolo
genesis exec-policy preset cautious --json

genesis exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `genesis exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `genesis approvals set --gateway`
or `genesis approvals set --node <id|name|ip>`.
...
`genesis approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
This changes the **host approvals file** only. To keep the requested Genesis policy aligned, also set:
