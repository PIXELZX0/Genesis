---
summary: "CLI reference for `genesis qr` (generate mobile pairing QR + setup code)"
read_when:
...
genesis qr --json
genesis qr --remote
genesis qr --url wss://gateway.example/ws
```
...
- With `--remote`, Genesis requires either `gateway.remote.url` or
