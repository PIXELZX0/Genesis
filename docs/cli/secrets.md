---
summary: "CLI reference for `genesis secrets` (reload, audit, configure, apply)"
read_when:
...
Use `genesis secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
genesis secrets reload
genesis secrets reload --json
genesis secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Genesis state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `genesis.json` refs)
...
```bash
genesis secrets audit
genesis secrets audit --check
genesis secrets audit --json
genesis secrets audit --allow-exec
```
...
```bash
genesis secrets configure
genesis secrets configure --plan-out /tmp/genesis-secrets-plan.json
genesis secrets configure --apply --yes
genesis secrets configure --providers-only
genesis secrets configure --skip-provider-setup
genesis secrets configure --agent ops
genesis secrets configure --json
```
...
- `configure` targets secret-bearing fields in `genesis.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Genesis fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
genesis secrets apply --from /tmp/genesis-secrets-plan.json --json
```
...
- `genesis.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.genesis/.env` known secret keys whose values were migrated
