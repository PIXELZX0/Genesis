...
### `genesis devices remove <deviceId>`
...
```
genesis devices remove <deviceId>
genesis devices remove <deviceId> --json
```

### `genesis devices clear --yes [--pending]`

Clear paired devices in bulk.

```
genesis devices clear --yes
genesis devices clear --yes --pending
genesis devices clear --yes --pending --json
```

### `genesis devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Genesis only prints the selected pending
...
key), Genesis supersedes the previous pending entry and issues a new
`requestId`. Run `genesis devices list` right before approval to use the
current ID.

If the device is already paired and asks for broader scopes or a broader role,
Genesis keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `genesis devices list`
or use `genesis devices approve --latest` to preview the exact upgrade before
approving it.
...
```bash
genesis devices rotate --device <deviceId> --role operator
```
