---
summary: "Step-by-step guide to building a messaging channel plugin for Genesis"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Genesis to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Genesis to a
...
<Info>
  If you have not built any Genesis plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Genesis keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `genesis/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `genesis/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `genesis/plugin-sdk/approval-auth-runtime`
- `genesis/plugin-sdk/approval-client-runtime`
- `genesis/plugin-sdk/approval-delivery-runtime`
- `genesis/plugin-sdk/approval-gateway-runtime`
- `genesis/plugin-sdk/approval-handler-adapter-runtime`
- `genesis/plugin-sdk/approval-handler-runtime`
- `genesis/plugin-sdk/approval-native-runtime`
- `genesis/plugin-sdk/approval-reply-runtime`
- `genesis/plugin-sdk/channel-runtime-context`

Likewise, prefer `genesis/plugin-sdk/setup-runtime`,
`genesis/plugin-sdk/setup-adapter-runtime`,
`genesis/plugin-sdk/reply-runtime`,
`genesis/plugin-sdk/reply-dispatch-runtime`,
`genesis/plugin-sdk/reply-reference`, and
`genesis/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `genesis/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `genesis/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `genesis/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `genesis.setupEntry` in
...
- use the broader `genesis/plugin-sdk/setup` seam only when you also need the
...
- `genesis/plugin-sdk/account-core`,
  `genesis/plugin-sdk/account-id`,
  `genesis/plugin-sdk/account-resolution`, and
  `genesis/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `genesis/plugin-sdk/inbound-envelope` and
  `genesis/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `genesis/plugin-sdk/messaging-targets` for target parsing/matching
- `genesis/plugin-sdk/outbound-media` and
  `genesis/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `genesis/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `genesis/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `genesis/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `genesis/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `genesis/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `genesis/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`genesis/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`genesis/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#genesis-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/genesis-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "genesis": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Genesis to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { GenesisConfig } from "genesis/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "genesis/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Genesis
  ...
  <Step title="Add a setup entry">
    ...
    Genesis loads this instead of the full entry when the channel is disabled
    ...
    `genesis/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Genesis. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Genesis.
...
├── package.json              # genesis.channel metadata
├── genesis.plugin.json      # Manifest with config schema
