...
read_when:
  ...
  - You are defining plugin config schemas or package.json genesis metadata
---

Reference for plugin packaging (`package.json` metadata), manifests
(`genesis.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `genesis` field that tells the plugin system what
...
```json genesis-clawhub-package.json
{
...
`genesis.channel` is cheap package metadata for channel discovery and setup
...
`genesis.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Genesis version in the form `>=x.y.z`.                         |
...
Interactive onboarding also uses `genesis.install` for install-on-demand
...
to show" metadata in `genesis.plugin.json` and the "how to install it"
...
```json
{
  "genesis": {
    "install": {
      "npmSpec": "@wecom/wecom-genesis-plugin@1.2.3",
...
still fails closed and tells the operator to run `genesis doctor --fix`.
...
When enabled, Genesis loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `genesis.plugin.json` in the package root.
Genesis uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Genesis",
  "configSchema": {
    "type": "object",
...
Genesis loads when it only needs setup surfaces (onboarding, config repair,
...
`genesis/plugin-sdk/channel-entry-contract` instead of
...
**When Genesis uses `setupEntry` instead of the full entry:**
...
```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "genesis/plugin-sdk/channel-config-schema";
...
mirror the generated JSON Schema into `genesis.plugin.json#channelConfigs` so
config schema, setup, and UI surfaces can inspect `channels.<id>` without
loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `genesis onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "genesis/plugin-sdk/channel-setup";
...
helpers from `genesis/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`genesis/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `genesis/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "genesis/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/genesis-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
genesis plugins install @myorg/genesis-my-plugin
```

Genesis tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
genesis plugins install clawhub:@myorg/genesis-my-plugin   # ClawHub only
```
...
```bash
genesis plugins install <package-name>
```

<Info>
  For npm-sourced installs, `genesis plugins install` runs
  `npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
  trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>

Bundled Genesis-owned plugins are the only startup repair exception: when a
