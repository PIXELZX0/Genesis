---
summary: "Run Genesis embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
...
The bundled `codex` plugin lets Genesis run embedded agent turns through the
...
Genesis still owns chat channels, session files, model selection, tools,
...
Native Codex turns keep Genesis plugin hooks as the public compatibility layer.
These are in-process Genesis hooks, not Codex `hooks.json` command hooks:
...
Genesis dynamic tool results after Genesis executes the tool and before the
result is returned to Codex. This is separate from the public
`tool_result_persist` plugin hook, which transforms Genesis-owned transcript
tool-result writes.
...
`embeddedHarness.runtime: "codex"` or `GENESIS_AGENT_RUNTIME=codex` when they
...
| ----------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------- |
| `openai/gpt-5.4`                                      | OpenAI provider through Genesis/PI plumbing | You want current direct OpenAI Platform API access with `OPENAI_API_KEY`. |
| `openai-codex/gpt-5.5`                                | OpenAI Codex OAuth through Genesis/PI       | You want ChatGPT/Codex subscription auth with the default PI runner.      |
...
GPT-5.5 is currently subscription/OAuth-only in Genesis. Use
...
Genesis records the selected harness id on that session and keeps using it for
later turns in the same session id. Change `embeddedHarness` config or
`GENESIS_AGENT_RUNTIME` when you want future sessions to use another harness;
...
`Runtime: Genesis Pi Default`, and the Codex app-server harness appears as
`Runtime: OpenAI Codex`.

## Requirements

- Genesis with the bundled `codex` plugin available.
...
Genesis on the protocol surface it has been tested against.
...
that runtime is forced, Genesis still tries the Codex harness and fails closed
...
```bash
GENESIS_AGENT_RUNTIME=codex genesis gateway run
```

With Codex forced, Genesis fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start. Set
`GENESIS_AGENT_HARNESS_FALLBACK=pi` only if you intentionally want PI to handle
...
Genesis session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the Genesis session binding for that thread
...
By default, Genesis starts local Codex harness sessions in YOLO mode:
...
- `GENESIS_CODEX_APP_SERVER_BIN`
- `GENESIS_CODEX_APP_SERVER_ARGS`
- `GENESIS_CODEX_APP_SERVER_MODE=yolo|guardian`
- `GENESIS_CODEX_APP_SERVER_APPROVAL_POLICY`
- `GENESIS_CODEX_APP_SERVER_SANDBOX`

`GENESIS_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`GENESIS_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
Model switching stays Genesis-controlled. When an Genesis session is attached
...
generic and works on any channel that supports Genesis text commands.
...
- `/codex resume <thread-id>` attaches the current Genesis session to an existing Codex thread.
...
normal turns. On the next message, Genesis resumes that Codex thread, passes the
currently selected Genesis model into app-server, and keeps extended history
enabled.
...
| Layer                                 | Owner                   | Purpose                                                             |
| ------------------------------------- | ----------------------- | ------------------------------------------------------------------- |
| Genesis plugin hooks                  | Genesis                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | Genesis bundled plugins | Per-turn adapter behavior around Genesis dynamic tools.             |
| Codex native hooks                    | Codex                   | Low-level Codex lifecycle and native tool policy from Codex config. |

Genesis does not use project or global Codex `hooks.json` files to route
Genesis plugin behavior. For the supported native tool and permission bridge,
Genesis injects per-thread Codex config for `PreToolUse`, `PostToolUse`, and
...
as Genesis plugin hooks in the v1 contract.

For Genesis dynamic tools, Genesis executes the tool after Codex asks for the
call, so Genesis fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
Genesis can mirror selected events, but it cannot rewrite the native Codex
...
notifications and Genesis adapter state, not native Codex hook commands.
Genesis's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke Genesis plugin hooks.

## V1 support contract

Codex mode is not PI with a different model call underneath. Codex owns more of
the native model loop, and Genesis adapts its plugin and session surfaces
...
| Genesis channel routing and delivery    | Supported                               | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                           |
| Genesis dynamic tools                   | Supported                               | Codex asks Genesis to execute these tools, so Genesis stays in the execution path.                                                         |
| Prompt and context plugins              | Supported                               | Genesis builds prompt overlays and projects context into the Codex turn before starting or resuming the thread.                            |
...
| Dynamic tool hooks                      | Supported                               | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Genesis-owned dynamic tools.                                  |
...
| Native permission policy                | Supported through the native hook relay | Codex `PermissionRequest` can be routed through Genesis policy where the runtime exposes it.                                               |
| App-server trajectory capture           | Supported                               | Genesis records the request it sent to app-server and the app-server notifications it receives.                                            |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                    | Future path                                                                                              |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Genesis does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                                           |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Genesis owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                                   |
| `tool_result_persist` for Codex-native tool records | That hook transforms Genesis-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.                             |
| Rich native compaction metadata                     | Genesis observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                                    |
| Compaction intervention                             | Current Genesis compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction.                |
| Stop or final-answer gating                         | Codex has native stop hooks, but Genesis does not expose final-answer gating as a v1 plugin contract.                                          | Future opt-in primitive with loop and timeout safeguards.                                                |
| Native MCP hook parity as a committed v1 surface    | The relay is generic, but Genesis has not version-gated and tested native MCP pre/post hook behavior end to end.                               | Add Genesis MCP relay tests and docs once the supported app-server protocol floor covers those payloads. |
| Byte-for-byte model API request capture             | Genesis can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                                  |

## Tools, media, and compaction

The Codex harness changes the low-level embedded agent executor only.

Genesis still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal Genesis delivery path.

The native hook relay is intentionally generic, but the v1 support contract is
limited to the Codex-native tool and permission paths that Genesis tests. Do not
assume every future Codex hook event is an Genesis plugin surface until the
runtime contract names it.

Codex MCP tool approval elicitations are routed through Genesis's plugin
...
delegated to Codex app-server. Genesis keeps a transcript mirror for channel
...
reasoning or plan records when the app-server emits them. Today, Genesis only
...
Genesis is writing an Genesis-owned session transcript tool result.
...
**Genesis uses PI instead of Codex:** `runtime: "auto"` can still use PI as the
