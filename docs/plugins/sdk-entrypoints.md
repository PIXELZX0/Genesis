...
when Genesis loads an installed package and let npm packages avoid runtime
TypeScript compilation. If an installed package only declares a TypeScript
source entry, Genesis will use a matching built `dist/*.js` peer when one
...
**Import:** `genesis/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                       | No       | —                   |
| `configSchema` | `GenesisPluginConfigSchema \| () => GenesisPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: GenesisPluginApi) => void`                              | Yes      | —                   |

- `id` must match your `genesis.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Genesis resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `genesis/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: GenesisPluginApi) => void`                              | No       | —                   |
| `registerFull`        | `(api: GenesisPluginApi) => void`                              | No       | —                   |
...
- Discovery registration is non-activating, not import-free. Genesis may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Genesis
...
Genesis loads this instead of the full entry when a channel is disabled,
...
- `genesis/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `genesis/plugin-sdk/channel-setup` for optional-install setup surfaces
- `genesis/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`genesis/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so Genesis can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Genesis to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Genesis rejects descriptor names outside
...
Genesis classifies loaded plugins by their registration behavior:
...
Use `genesis plugins inspect <id>` to see a plugin's shape.
