---
summary: "Install and use Codex, Claude, and Cursor bundles as Genesis plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Genesis maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Genesis can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Genesis maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Genesis plugins. Native plugins run
...
of requiring authors to rewrite them as native Genesis plugins, Genesis
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    genesis plugins install ./my-bundle

    # Archive
    genesis plugins install ./my-bundle.tgz
...
## What Genesis maps from bundles

Not every bundle feature runs in Genesis today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Genesis skills                                            | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Genesis-style `HOOK.md` + `handler.ts` layouts                                              | Codex          |
...
- bundle skill roots load as normal Genesis skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Genesis skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Genesis hook-pack
...
- Genesis merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Genesis exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Genesis uses `sse`
...
Genesis registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Genesis sanitizes shell override keys before applying them
...
- Genesis loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `genesis plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Genesis does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Genesis best when they use skill roots and Genesis-style
...
- Bundled plugin runtime dependencies ship inside the Genesis package under
  `dist/*`. Genesis does **not** run `npm install` at startup for bundled
...
- Genesis does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `genesis plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Genesis does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Genesis hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
