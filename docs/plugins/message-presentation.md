...
Message presentation is Genesis's shared contract for rich outbound chat UI.
...
```bash
genesis message send --channel slack \
  --target channel:C123 \
...
```bash
genesis message send --channel telegram \
...
Use helpers from `genesis/plugin-sdk/interactive-runtime` when bridging older
code:
