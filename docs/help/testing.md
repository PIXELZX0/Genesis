...
Genesis has three Vitest suites (unit/integration, e2e, live) and a small set
...
- Linux VM-backed QA lane: `pnpm genesis qa suite --runner multipass --scenario channel-chat-baseline`
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `GENESIS_LIVE_MODEL_FILE_PROBE=0` or
    `GENESIS_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Genesis Scheduled Live And E2E Checks` and manual
    `Genesis Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `Genesis Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/genesis-live-and-e2e-checks-reusable.yml` and its
...
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `genesis models list --provider moonshot --json`, then run an isolated
  `genesis agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
Convex-managed live Telegram lane as parallel jobs. `Genesis Release Checks`
runs the same lanes before release approval.

- `pnpm genesis qa suite`
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm genesis qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  - Builds an npm tarball from the current checkout, installs it globally in
    ...
    endpoint.
  - Use `GENESIS_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:npm-telegram-live`
  ...
  - Uses the same Telegram env credentials or Convex credential source as
    ...
    `GENESIS_QA_CONVEX_SITE_URL` and the role secret. If
    `GENESIS_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
    the Docker wrapper selects Convex automatically.
  - `GENESIS_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `GENESIS_QA_CREDENTIAL_ROLE` for this lane only.
...
- `pnpm test:docker:bundled-channel-deps`
  - Packs and installs the current Genesis build in Docker, starts the Gateway
  ...
  - Also installs a known older npm baseline, enables Telegram before running
    `genesis update --tag <candidate>`, and verifies the candidate's
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests. Each
    ...
    the installed `genesis update` command in the same guest and verifies the
  ...
  - The script writes nested lane logs under `/tmp/genesis-parallels-npm-update.*`.
...
- `pnpm genesis qa matrix`
  ...
  - This QA host is repo/dev-only today. Packaged Genesis installs do not ship
    `qa-lab`, so they do not expose `genesis qa`.
  ...
  - Uses the pinned stable Tuwunel image `ghcr.io/matrix-construct/tuwunel:v1.5.1` by default. Override with `GENESIS_QA_MATRIX_TUWUNEL_IMAGE` when you need to test a different image.
  ...
  - Emits progress by default and enforces a hard run timeout with `GENESIS_QA_MATRIX_TIMEOUT_MS` (default 30 minutes). Cleanup is bounded by `GENESIS_QA_MATRIX_CLEANUP_TIMEOUT_MS` and failures include the recovery `docker compose ... down --remove-orphans` command.
- `pnpm genesis qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `GENESIS_QA_TELEGRAM_GROUP_ID`, `GENESIS_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `GENESIS_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `GENESIS_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
  - Exits non-zero when any scenario fails. Use `--allow-failures` when you
    want artifacts without a failing exit code.
...
When `--credential-source convex` (or `GENESIS_QA_CREDENTIAL_SOURCE=convex`) is enabled for
`genesis qa telegram`, QA lab acquires an exclusive lease from a Convex-backed pool, heartbeats
...
- `GENESIS_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `GENESIS_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `GENESIS_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `GENESIS_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `GENESIS_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `GENESIS_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `GENESIS_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `GENESIS_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `GENESIS_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `GENESIS_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `GENESIS_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`GENESIS_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`GENESIS_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm genesis qa credentials doctor
pnpm genesis qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm genesis qa credentials list --kind telegram
pnpm genesis qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`GENESIS_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
- how `genesis qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `genesis qa <runner>` instead of registering a competing root command.
   Runner plugins should declare `qaRunners` in `genesis.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`.
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-genesis-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `GENESIS_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
  ...
  <Accordion title="Fast local iteration">
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `GENESIS_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `GENESIS_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Scope:
  ...
  - Exercises Genesis's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `GENESIS_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `GENESIS_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.genesis`.
- Set `GENESIS_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `GENESIS_LIVE_TEST_QUIET=0` if you want the full startup logs back.
...
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `GENESIS_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `GENESIS_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `GENESIS_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `GENESIS_LIVE_GATEWAY_SMOKE=1`,
  `GENESIS_LIVE_GATEWAY_MAX_MODELS=8`,
  `GENESIS_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `GENESIS_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
  explicitly want the larger exhaustive scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, then reuses it for the live Docker lanes. It also builds one shared `scripts/e2e/Dockerfile` image via `test:docker:e2e-build` and reuses it for the E2E container smoke runners that exercise the built app. The aggregate uses a weighted local scheduler: `GENESIS_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. Defaults are 10 slots, `GENESIS_DOCKER_ALL_LIVE_LIMIT=6`, `GENESIS_DOCKER_ALL_NPM_LIMIT=8`, and `GENESIS_DOCKER_ALL_SERVICE_LIMIT=7`; tune `GENESIS_DOCKER_ALL_WEIGHT_LIMIT` or `GENESIS_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Genesis E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `GENESIS_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker.
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Genesis tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, verifies doctor repairs activated plugin runtime deps, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `GENESIS_NPM_ONBOARD_PACKAGE_TGZ=/path/to/genesis-*.tgz`, skip the host rebuild with `GENESIS_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `GENESIS_NPM_ONBOARD_CHANNEL=discord`.
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `genesis infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `GENESIS_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/genesis-*.tgz`, skip the host build with `GENESIS_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `GENESIS_BUN_GLOBAL_SMOKE_DIST_IMAGE=genesis-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `GENESIS_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `GENESIS_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `GENESIS_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=genesis-dockerfile-smoke:local GENESIS_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Bundled plugin runtime deps: `pnpm test:docker:bundled-channel-deps` builds a small Docker runner image by default, builds and packs Genesis once on the host, then mounts that tarball into each Linux install scenario. Reuse the image with `GENESIS_SKIP_DOCKER_BUILD=1`, skip the host rebuild after a fresh local build with `GENESIS_BUNDLED_CHANNEL_HOST_BUILD=0`, or point at an existing tarball with `GENESIS_BUNDLED_CHANNEL_PACKAGE_TGZ=/path/to/genesis-*.tgz`. The full Docker aggregate pre-packs this tarball once, then shards bundled channel checks into independent lanes, including separate update lanes for Telegram, Discord, Slack, Feishu, memory-lancedb, and ACPX. Use `GENESIS_BUNDLED_CHANNELS=telegram,slack` to narrow the channel matrix when running the bundled lane directly, or `GENESIS_BUNDLED_CHANNEL_UPDATE_TARGETS=telegram,acpx` to narrow the update scenario. The lane also verifies that `channels.<id>.enabled=false` and `plugins.entries.<id>.enabled=false` suppress doctor/runtime-dependency repair.
- Narrow bundled plugin runtime deps while iterating by disabling unrelated scenarios, for example:
  `GENESIS_BUNDLED_CHANNEL_SCENARIOS=0 GENESIS_BUNDLED_CHANNEL_UPDATE_SCENARIO=0 GENESIS_BUNDLED_CHANNEL_ROOT_OWNED_SCENARIO=0 GENESIS_BUNDLED_CHANNEL_SETUP_ENTRY_SCENARIO=0 pnpm test:docker:bundled-channel-deps`.

To prebuild and reuse the shared built-app image manually:

```bash
GENESIS_DOCKER_E2E_IMAGE=genesis-docker-e2e:local pnpm test:docker:e2e-build
GENESIS_DOCKER_E2E_IMAGE=genesis-docker-e2e:local GENESIS_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `GENESIS_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `GENESIS_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__genesis_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `GENESIS_SKIP_CHANNELS=1` so gateway live probes do not start
...
`GENESIS_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Genesis gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `genesis/default`, then sends a
...
This lane expects a usable live model key, and `GENESIS_PROFILE_FILE`
...
container, starts a second container that spawns `genesis mcp serve`, then
...
- `GENESIS_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `GENESIS_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `GENESIS_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/genesis/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `GENESIS_LIVE_PROVIDERS` / `GENESIS_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `GENESIS_DOCKER_AUTH_DIRS=all`, `GENESIS_DOCKER_AUTH_DIRS=none`, or a comma list like `GENESIS_DOCKER_AUTH_DIRS=.claude,.codex`
- `GENESIS_LIVE_GATEWAY_MODELS=...` / `GENESIS_LIVE_MODELS=...` to narrow the run
- `GENESIS_LIVE_GATEWAY_PROVIDERS=...` / `GENESIS_LIVE_PROVIDERS=...` to filter providers in-container
- `GENESIS_SKIP_DOCKER_BUILD=1` to reuse an existing `genesis:local-live` image for reruns that do not need a rebuild
- `GENESIS_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `GENESIS_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `GENESIS_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
