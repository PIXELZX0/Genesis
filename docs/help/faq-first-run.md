...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    [https://github.com/PIXELZX0/Genesis/issues](https://github.com/PIXELZX0/Genesis/issues)
    [https://github.com/PIXELZX0/Genesis/pulls](https://github.com/PIXELZX0/Genesis/pulls)
  ...
  <Accordion title="Recommended way to install and set up Genesis">
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `GENESIS_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `GENESIS_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `genesis doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `genesis gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `genesis gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Genesis now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/PIXELZX0/Genesis/blob/main/CHANGELOG.md](https://github.com/PIXELZX0/Genesis/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.genesis.ai (SSL error)">
    ...
    [https://github.com/PIXELZX0/Genesis/tree/main/docs](https://github.com/PIXELZX0/Genesis/tree/main/docs)
  ...
  <Accordion title="Windows install says git not found or genesis not recognized">
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Genesis, track/report it in:

    - [Issue #30640](https://github.com/PIXELZX0/Genesis/issues/30640)
  ...
  <Accordion title="How do I install Genesis on Linux?">
  ...
  <Accordion title="How do I install Genesis on a VPS?">
  ...
  <Accordion title="Can I ask Genesis to update itself?">
  ...
  <Accordion title="What does onboarding actually do?">
    ...
    - **Channels** (WhatsApp, Telegram, Discord, Mattermost, Signal, iMessage, plus bundled channel plugins)
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    ...
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    For Anthropic in Genesis, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Genesis**: Anthropic staff
      told us this usage is allowed again, and Genesis is treating `claude -p`
    ...
    Genesis also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Genesis-style Claude CLI usage is allowed again, so
    Genesis treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Genesis treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Genesis token path, but Genesis now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Genesis, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Genesis Claude-login path with Extra Usage enabled).
  ...
  <Accordion title="Why does Genesis still mention openai-codex?">
    ...
    `genesis models auth login --provider openai-codex` and use
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Genesis can show the currently visible provider usage/quota windows in
    `genesis models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Genesis fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Genesis. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    2. Enable the plugin: `genesis plugins enable google`
    3. Login: `genesis models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Genesis needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="If I buy a Mac mini to run Genesis, can I connect it to my MacBook Pro?">
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `genesis doctor --fix` can try to resolve them.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Genesis instances?">
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Genesis-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Genesis on a dedicated machine?">
  ...
  <Accordion title="Can I run Genesis in a VM and what are the requirements?">
