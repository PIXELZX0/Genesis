...
Genesis keeps `src/cli/debug-timing.ts` as a small helper for local
...
`genesis models list`, a temporary patch in
...
```bash
GENESIS_DEBUG_TIMING=1 pnpm genesis models list --all --provider moonshot
```

Example output from a temporary `models list` investigation:

```text
Genesis CLI debug timing: models list
...
```bash
GENESIS_DEBUG_TIMING=json pnpm genesis models list --all --provider moonshot \
...
extension `package.json` and `genesis.plugin.json` metadata, `tsconfig.json`,
