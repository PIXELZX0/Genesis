...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    ...
    Models are referenced as `provider/model` (example: `openai/gpt-5.4` or `openai-codex/gpt-5.5`). If you omit the provider, Genesis first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Genesis falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - edit `agents.defaults.model` in `~/.genesis/genesis.json`
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `genesis onboard` and choose `Ollama`
    ...
    - for manual switching, use `genesis models list` and `genesis models set ollama/<model>`
  ...
  <Accordion title="What do Genesis, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `genesis models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Genesis release (or run from source `main`), then restart the gateway.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    The rate-limit bucket includes more than plain `429` responses. Genesis
    ...
    explicit billing text on `401` or `403`, Genesis can still keep that in
    ...
    `organization spending limit exceeded`), Genesis treats it as
    ...
    unknown/error in it". Genesis does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `genesis models auth login --provider anthropic --method cli --set-default` on the gateway host.
