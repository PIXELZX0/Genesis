...
Current Genesis releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`genesis onboard`) and `genesis channels add` still surface
...
```bash
genesis plugins install --link <path-to-local-nostr-plugin>
```
...
genesis channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
