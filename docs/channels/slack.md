...
```json
{
  ...
  "features": {
    "bot_user": { "display_name": "Genesis", "always_online": true },
    "app_home": {
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Genesis is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are Genesis-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Genesis falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `genesis channels status --probe --json` shows `botTokenStatus` or
