...
genesis config set channels.discord.enabled true --strict-json
genesis gateway
```

    If Genesis is already running as a background service, restart it via the Genesis Mac app or by stopping and restarting the `genesis gateway run` process.
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    Genesis can stream draft replies by sending a temporary message and editing it as text arrives. `channels.discord.streaming` takes `off` (default) | `partial` | `block` | `progress`. `progress` maps to `partial` on Discord; `streamMode` is a legacy alias and is auto-migrated.
...
```
    ...
    Preview streaming is text-only; media replies fall back to normal delivery. When `block` streaming is explicitly enabled, Genesis skips the preview stream to avoid double-streaming.
...
```
    ...
    - `spawnAcpSessions` is only required when Genesis needs to create/bind a child thread via `--thread auto|here`.
...
```
  ...
  <Accordion title="Approvals in Discord">
    ...
    When those buttons are present, they are the primary approval UX; Genesis
...
Genesis uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
- Genesis also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio. Genesis generates the waveform automatically, but needs `ffmpeg` and `ffprobe` on the gateway host to inspect and convert.

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord rejects text + voice message in the same payload).
- Any audio format is accepted; Genesis converts to OGG/Opus as needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep Genesis current (`genesis update`) so the Discord voice receive recovery logic is present
