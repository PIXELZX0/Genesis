...
Nextcloud Talk ships as a bundled plugin in current Genesis releases, so
...
```bash
genesis plugins install ./path/to/local/nextcloud-talk-plugin
```
...
4. Configure Genesis:
   ...
   ```bash
   genesis channels add --channel nextcloud-talk \
