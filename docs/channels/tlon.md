...
Tlon ships as a bundled plugin in current Genesis releases, so normal packaged
...
```bash
genesis plugins install ./path/to/local/tlon-plugin
```
...
By default, Genesis blocks private/internal hostnames and IP ranges for SSRF protection.
