...
Mattermost ships as a bundled plugin in current Genesis releases, so normal
...
```bash
genesis plugins install ./path/to/local/mattermost-plugin
```
...
Native slash commands are opt-in. When enabled, Genesis registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Genesis derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Genesis registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Genesis.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Genesis.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Genesis, not `404`.
...
Genesis resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Genesis sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Genesis sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If the stream cannot be finalized in place (for example the post was deleted mid-stream), Genesis falls back to sending a fresh final post so the reply is never lost.
...
- If `interactions.callbackBaseUrl` is omitted, Genesis derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Genesis run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="genesis-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"genesis-mattermost-interactions",
...
`genesis message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Genesis
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Genesis. Set an
