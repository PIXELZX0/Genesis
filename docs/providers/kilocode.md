---
summary: "Use Kilo Gateway's unified API to access many models in Genesis"
title: "Kilocode"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Genesis
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    genesis onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    genesis models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Genesis treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Genesis.
</Note>

## Built-in catalog

Genesis dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Genesis queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Genesis keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Genesis falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.genesis/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Genesis configuration reference.
  </Card>
