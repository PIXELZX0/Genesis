    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        genesis onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        genesis models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        GENESIS_CONFIG_PATH=/tmp/genesis-kimi/genesis.json \
        GENESIS_STATE_DIR=/tmp/genesis-kimi \
        genesis agent --local \
    ...
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      ...
      <Step title="Verify the model is available">
        ```bash
        genesis models list --provider kimi
        ```
      </Step>
    </Steps>
...
Genesis also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    Genesis also maps runtime `/think` levels for Moonshot:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Genesis normalizes incompatible `tool_choice` values to `auto` for compatibility.
    </Warning>
    ...
    default strategy. Genesis only forwards `thinking.keep` for
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves tool_call ids shaped like `functions.<name>:<index>`. Genesis preserves them unchanged so multi-turn tool calls keep working.
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. Genesis keys that off endpoint
