...
Vercel AI Gateway also serves TypeSafe Jev (`typesafe-ai/jev`), an evaluation
model that cannot chat, so discovery hides it from the chat model list. The
[TypeSafe Jev plugin](/tools/jev) can use your `AI_GATEWAY_API_KEY` to reach it.
