---
summary: "Run Genesis through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Genesis against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Genesis compat flags for inferrs
title: "Inferrs"
---
...
<Steps>
  ...
  <Step title="Add an Genesis provider entry">
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but genesis infer model run fails">
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running Genesis against local model servers.
  </Card>
