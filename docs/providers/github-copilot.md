  ...
  <Tab title="Built-in provider (github-copilot)">
    ...
    <Steps>
      ...
      <Step title="Set a default model">
        ```bash
        genesis models set github-copilot/claude-opus-4.7
        ```
...
genesis models auth login --provider github-copilot --method device --set-default
```
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Genesis
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Genesis sends Copilot IDE-style request headers on Copilot transports,
  ...
  <Accordion title="Environment variable resolution order">
    Genesis resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Genesis uses the highest-priority one.
    The device-login flow (`genesis models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Genesis runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Genesis can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Genesis discovers available
...
1. Genesis resolves your GitHub token (from env vars or auth profile).
...
available, Genesis skips Copilot and tries the next provider.
