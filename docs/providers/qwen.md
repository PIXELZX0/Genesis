---
summary: "Use Qwen Cloud via Genesis's bundled qwen provider"
read_when:
...
See [Issue #49557](https://github.com/PIXELZX0/Genesis/issues/49557) for
background.

</Warning>

Genesis now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        genesis onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        genesis onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        genesis models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        genesis onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        genesis onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Genesis currently ships this bundled Qwen catalog. The configured catalog is
...
<AccordionGroup>
  ...
  <Accordion title="Video generation details">
    For video generation, Genesis maps the configured Qwen region to the matching
