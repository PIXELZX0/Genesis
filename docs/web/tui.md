...
- `genesis chat` and `genesis terminal` are aliases for `genesis tui --local`.
...
- Local shell commands receive `GENESIS_SHELL=tui-local` in their environment.
...
If `genesis config validate` is already failing, start with `genesis configure`
or `genesis doctor --fix` first. `genesis chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `genesis config set` or `genesis configure`, then rerun `!genesis config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!genesis doctor --fix`.

Tips:

- Prefer `genesis config set` or `genesis configure` over hand-editing `genesis.json`.
- `genesis docs "<query>"` searches the live docs index from the same machine.
- `genesis config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `GENESIS_THEME=light` before launching `genesis tui`.
- To force the original dark palette instead, set `GENESIS_THEME=dark`.
...
- [Config](/cli/config) — inspect, validate, and edit `genesis.json`
