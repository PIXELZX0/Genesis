...
Most operations flow through the Gateway (`genesis gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__genesis__/canvas/`, `/__genesis__/a2ui/`), protected by Gateway auth when bound beyond loopback.
