...
- The app installs a per‑user LaunchAgent labeled `ai.genesis.gateway`
  (or `ai.genesis.<profile>` when using `--profile`/`GENESIS_PROFILE`; legacy `com.genesis.*` is supported).
