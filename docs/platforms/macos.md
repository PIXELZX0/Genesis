...
- Installs the global CLI (`genesis`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).
...
The app manages a per‑user LaunchAgent labeled `ai.genesis.gateway`
(or `ai.genesis.<profile>` when using `--profile`/`GENESIS_PROFILE`; legacy `com.genesis.*` still unloads).
...
Replace the label with `ai.genesis.<profile>` when running a named profile.
...
Avoid putting your Genesis state dir in iCloud or other cloud-synced folders.
...
If `genesis doctor` detects state under:
