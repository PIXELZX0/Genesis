...
2. `npm i -g @pixelzx/genesis@latest`
...
service for shared or always-on servers. `genesis gateway install` and
`genesis onboard --install-daemon` already render the current canonical unit
...
sessions and channel connections. Genesis therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Genesis starts the child through a short
...
- Genesis-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `GENESIS_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
