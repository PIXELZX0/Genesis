...
Run `genesis devices list` again before approval.
...
- `GENESIS_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Genesis iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Genesis builds.
...
```bash
export GENESIS_APNS_TEAM_ID="TEAMID"
export GENESIS_APNS_KEY_ID="KEYID"
export GENESIS_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.genesis/credentials/apns
chmod 700 ~/.genesis/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.genesis/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.genesis/credentials/apns/AuthKey_KEYID.p8
export GENESIS_APNS_PRIVATE_KEY_PATH="$HOME/.genesis/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_genesis-gw._tcp` on `local.` and, when configured, the same
...
`genesis.internal.`) and Tailscale split DNS.
...
- Pairing prompt never appears: run `genesis devices list` and approve manually.
