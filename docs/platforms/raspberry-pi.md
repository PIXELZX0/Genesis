...
## 9) Access the Genesis Dashboard
...
```bash
ssh user@gateway-host 'genesis dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/genesis-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
- `GENESIS_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Genesis, add a service drop-in to reduce restart
...
systemctl --user restart genesis-gateway.service
```

If possible, keep Genesis state/cache on SSD-backed storage to avoid SD-card
...
# Enable if not
systemctl --user enable genesis-gateway.service
...
```bash
# Check logs
journalctl --user -u genesis-gateway.service --no-pager -n 100
