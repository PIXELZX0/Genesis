  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __GENESIS_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__GENESIS_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__GENESIS_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /genesis/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/genesis"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/genesis/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/genesis"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("genesis.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "genesis.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/genesis",
    ...
    const scopedKey = "genesis.control.settings.v1:wss://gateway.example:8443/genesis";
  ...
  it("does not persist gateway tokens when saving settings", async () => {
    ...
    const scopedKey = `genesis.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("falls back to claw when persisted custom theme data is invalid", async () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `genesis.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        theme: "custom",
        themeMode: "dark",
  ...
  it("caps persisted session scopes to the most recent gateways", async () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `genesis.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("persists local user identity separately from gateway settings", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("genesis.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "🦞",
    });
  });
  ...
  it("removes the persisted local user identity when cleared", async () => {
    ...
    expect(localStorage.getItem("genesis.control.user.v1")).toBeNull();
  });
});
