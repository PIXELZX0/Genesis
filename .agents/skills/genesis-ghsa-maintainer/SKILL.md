---
name: genesis-ghsa-maintainer
description: Inspect, patch, validate, publish, or confirm Genesis GHSA security advisories and private-fork state.
---

# Genesis GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `genesis-release-maintainer`.
...
```bash
fork=$(gh api /repos/genesis/genesis/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/genesis/genesis/security-advisories/<GHSA> \
