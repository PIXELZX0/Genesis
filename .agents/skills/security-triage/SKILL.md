...
description: Triage Genesis security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---

# Security Triage

Use when reviewing Genesis security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/genesis/genesis/security-advisories/<GHSA>`.
...
```bash
gh api /repos/genesis/genesis/security-advisories/<GHSA>
gh api /repos/genesis/genesis/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view genesis version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo genesis/genesis --match title,body,comments -- "<terms>"
gh search prs --repo genesis/genesis --match title,body,comments -- "<terms>"
```
