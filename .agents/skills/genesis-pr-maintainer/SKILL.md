---
name: genesis-pr-maintainer
description: Review, triage, close, label, comment on, or land Genesis PRs/issues with maintainer evidence checks.
---

# Genesis PR Maintainer
...
- Use `--repo genesis/genesis` with `--match title,body` first.
...
```bash
gh search prs --repo genesis/genesis --match title,body --limit 50 -- "auto-update"
gh search issues --repo genesis/genesis --match title,body --limit 50 -- "auto-update"
gh search issues --repo genesis/genesis --match title,body --limit 50 \
