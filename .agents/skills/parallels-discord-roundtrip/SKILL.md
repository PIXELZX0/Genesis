...
```bash
export GENESIS_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.genesis/genesis.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env GENESIS_PARALLELS_DISCORD_TOKEN \
...
- Use the `genesis` wrapper for guest `message send/read`; `node genesis.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/genesis-parallels-smoke.*`
...
- host inbound nonce appears in `genesis message read` output
