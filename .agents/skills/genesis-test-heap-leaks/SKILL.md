---
name: genesis-test-heap-leaks
description: Investigate Genesis pnpm test memory growth, Vitest OOMs, RSS spikes, and heap snapshot deltas.
---

# Genesis Test Heap Leaks
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && GENESIS_TEST_MEMORY_TRACE=1 GENESIS_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 GENESIS_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap GENESIS_TEST_WORKERS=2 GENESIS_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `GENESIS_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
2. Wait for repeated snapshots before concluding anything.
   ...
   - Use `.agents/skills/genesis-test-heap-leaks/scripts/heapsnapshot-delta.mjs` to compare either two files directly or the earliest/latest pair per PID in one lane directory.
...
- Direct comparison:
  - `node .agents/skills/genesis-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/genesis-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast-batch-2`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
