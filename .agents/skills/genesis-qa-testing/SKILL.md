---
name: genesis-qa-testing
description: Run, watch, debug, extend, or explain Genesis qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
GENESIS_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
...
5. If the user wants to watch the live UI, find the current `genesis-qa` listen port and report `http://127.0.0.1:<port>`.
...
- That item is the first place to look for:
  ...
  - `GENESIS_QA_PROVIDER_MODE`
  - `GENESIS_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Genesis`
  - item: `GENESIS_QA_CONVEX_SITE_URL`
  - item: `GENESIS_QA_CONVEX_SECRET_MAINTAINER`
  - item: `GENESIS_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `GENESIS QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `GENESIS_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `GENESIS_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Genesis/GENESIS_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
GENESIS_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
GENESIS_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
GENESIS_QA_PROVIDER_MODE="mock-openai" \
GENESIS_NPM_TELEGRAM_PACKAGE_SPEC="genesis@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `GENESIS_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `GENESIS_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `GENESIS_QA_PROVIDER_MODE`, map it explicitly to `GENESIS_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    - `GENESIS_QA_CREDENTIAL_SOURCE=convex`
    - `GENESIS_QA_CREDENTIAL_ROLE=maintainer`
    - `GENESIS_QA_CONVEX_SITE_URL`
    - `GENESIS_QA_CONVEX_SECRET_MAINTAINER`
    - `GENESIS_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Genesis identity combines with the character.
...
```bash
pnpm genesis qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `GENESIS_HOME` sandboxed.
